...
name|fail
argument_list|(
literal|"must fail with NullPointerException"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
parameter_list|)
block|{}
comment|//Validating negative clusterId
try|try
block|{
name|ns1
operator|.
...
parameter_list|)
block|{}
comment|//Validating recovery on active node
try|try
block|{
name|ns1
operator|.
...
name|fail
argument_list|(
literal|"must fail with IllegalStateException"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
argument_list|)
expr_stmt|;
comment|//Validate no. of affected paths in readOnlyMode
name|DocumentNodeStore
name|ns2
init|=
name|builderProvider
...
argument_list|)
expr_stmt|;
comment|//Validate no. of recovered paths
name|DocumentNodeStore
name|ns3
init|=
name|builderProvider
...
argument_list|)
expr_stmt|;
comment|//Validating recovery on non existing path
try|try
block|{
name|ns2
operator|.
...
specifier|public
name|void
name|lastRevisionUpdateOnNodeRestart
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
decl_stmt|;
comment|// DocumentNodeStore refreshes the head revision and _lastRev
comment|// when there was no commit for one minute
name|clock
operator|.
name|waitUntil
argument_list|(
...
name|assertThat
argument_list|(
literal|"lastRev must be greater or equal '"
operator|+
name|Utils
operator|.
...
name|assertNotEquals
argument_list|(
literal|"Last revision should be updated after 1 minute even if background thread is not running"
argument_list|,
name|beforeLastRev
argument_list|,
name|afterLastRev
...
expr_stmt|;
block|}
comment|// verify that the dispose method does a best effort in releasing resources (see OAK-7709)
annotation|@
name|Test
specifier|public
name|void
name|disposeAfterLeaseFailure
parameter_list|()
throws|throws
name|Exception
block|{
...
specifier|final
name|AtomicBoolean
name|documentStoreDisposeWasReached
init|=
operator|new
name|AtomicBoolean
argument_list|(
...
name|fail
argument_list|(
literal|"expected lease failure exception; test setup may be incorrect"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
parameter_list|)
block|{
comment|// due to lease update failure
block|}
name|assertTrue
argument_list|(
literal|"The DocumentStore instance's dispose() method was not called"
argument_list|,
name|documentStoreDisposeWasReached
operator|.
name|get
...
specifier|public
name|void
name|readOnlyOnEmptyDocumentStore
parameter_list|()
block|{
name|DocumentStore
name|store
...
parameter_list|)
block|{
comment|// must not hit last line of defence (ReadOnlyDocumentStoreWrapper)
name|assertFalse
argument_list|(
name|Throwables
operator|.
...
block|}
decl_stmt|;
comment|// set batch size to half the update limit
name|int
name|batchSize
init|=
