...
begin_comment
comment|/**  * This class maintains a list of local changes (path+local revision timestamp), which  * shouldn't be read from the secondary Mongo, as we are not sure if they have  * been already replicated from primary. Once we get this confidence, the entry  * will be removed from the map.  */
end_comment
...
literal|100
decl_stmt|;
comment|/**      * This map contains document paths and timestamps in which they have been      * changed. Paths in this collection hasn't been replicated to secondary      * instances yet.      */
specifier|final
name|Map
argument_list|<
name|String
...
argument_list|()
decl_stmt|;
comment|/**      * If there's more than {@link #SIZE_LIMIT} paths in the      * {@link #localChanges}, the class will clear the above map and update this      * variable with the timestamp of the last local change. {@code true} will be returned      * for all {@link #mayContainChildrenOf(String)} and {@link #mayContain(String)}      * invocations until this timestamp is replicated to all secondary instances.      *<p>      * This is a safety mechanism, so the {@link #localChanges} won't grow too much.      */
specifier|private
specifier|volatile
name|long
name|latestChange
...
block|}
block|}
comment|/**      * Check if it's possible that the children of the given document hasn't      * been replicated to the secondary yet.      *      * @param parentId      * @return {@code true} if it's possible that the children of given document      *         are still in the Mongo replication queue      */
specifier|public
name|boolean
