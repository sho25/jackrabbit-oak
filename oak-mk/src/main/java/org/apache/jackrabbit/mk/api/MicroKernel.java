...
name|MicroKernelException
function_decl|;
comment|/**      * Returns the node tree rooted at the specified parent node with the      * specified depth, maximum child node count and offset. The depth of the      * returned tree is governed by the {@code depth} parameter:      *<table>      *<tr>      *<td>depth = 0</td>      *<td>properties, including {@code :childNodeCount} and      * child node names (i.e. empty child node objects)</td>      *</tr>      *<tr>      *<td>depth = 1</td>      *<td>properties, child nodes and their properties (including      * {@code :childNodeCount})</td>      *</tr>      *<tr>      *<td>depth = 2</td>      *<td>[and so on...]</td>      *</tr>      *</table>      *<p/>      * The {@code offset} and {@code count} parameters are only applied to the      * direct child nodes of the root of the returned node tree.      *<p/>      * The optional {@code filter} parameter allows to specify glob patterns for names of      * nodes and properties to be included or excluded.      *<p/>      * Example:      *<pre>      * {      *   nodes: [ "foo*", "-foo1" ],      *   properties: [ "*", "-:childNodeCount" ]      * }      *</pre>      * In the above example all child nodes with names starting with "foo" will      * included, except for nodes named "foo1"; similarly, all properties will      * be included except for the ":childNodeCount" meta data property.      *<p/>      * Glob Syntax:      *<ul>      *<li>a {@code nodes} or {@code properties} filter consists of one or more<i>globs</i>.</li>      *<li>a<i>glob</i> prefixed by {@code -} (dash) is treated as an exclusion pattern;      * all others are considered inclusion patterns.</li>      *<li>a leading {@code -} (dash) must be escaped by prepending {@code \} (backslash)      * if it should be interpreted as a literal.</li>      *<li>{@code *} (asterisk) serves as a<i>wildcard</i>, i.e. it matches any      * substring in the target name.</li>      *<li>{@code *} (asterisk) occurrences within the glob to be interpreted as      * literals must be escaped by prepending {@code \} (backslash).</li>      *<li>a filter matches a target name if any of the inclusion patterns match but      * none of the exclusion patterns.</li>      *</ul>      * If no filter is specified the implicit default filter is assumed:      * {@code {nodes:["*"],properties:["*"]}}      *<p/>      * For more information see {@link #getNodes(String, String)}.      *      * @param path       path denoting root of node tree to be retrieved      * @param revisionId revision id, if {@code null} the current head revision is assumed      * @param depth      maximum depth of returned tree      * @param offset     start position in the iteration order of child nodes (0 to start at the      *                   beginning)      * @param count      maximum number of child nodes to retrieve (-1 for all)      * @param filter     optional filter on property names; if {@code null} or      *                   {@code ""} the default filter will be assumed      * @return node tree in JSON format or {@code null} if the specified node does not exist      * @throws MicroKernelException if the specified revision does not exist or if another error occurs      * @see #getNodes(String, String)      */
name|String
comment|/* jsonTree */
name|getNodes
parameter_list|(
