...
condition|)
block|{
comment|// q = new WildcardQuery(newFulltextTerm(token));
comment|// PhraseQuery pq = new PhraseQuery();
comment|// pq.add(newFulltextTerm(token));
comment|// q = pq;
block|}
else|else
block|{
...
comment|// } else {
comment|// q = new WildcardQuery(newFulltextTerm(token
comment|// + WildcardQuery.WILDCARD_STRING));
comment|// }
block|}
name|qs
operator|.
