...
init|=
literal|"oak.fulltext.useActualEntryCount"
decl_stmt|;
specifier|private
specifier|static
specifier|final
...
operator|&&
operator|!
name|isPlanWithValidPathFilter
argument_list|()
condition|)
block|{
return|return
...
comment|// that lowest cost is returned
return|return
name|b
operator|.
name|setEstimatedEntryCount
...
parameter_list|()
block|{
comment|// With OAK-7947 lucene indexes return a non-null index node to delay reading index files
comment|// While IndexNode could have a status check method but for now we are using this work-around
comment|// to check null on {@code getIndexStatistics()} as proxy indicator
comment|// (this could be avoided by returning lazy statistics)
if|if
condition|(
name|indexNode
operator|.
...
name|warn
argument_list|(
literal|"Statistics not available - possibly index is corrupt? Returning high doc count"
argument_list|)
expr_stmt|;
return|return
name|Integer
...
argument_list|)
expr_stmt|;
comment|// can manage any order desc/asc
name|result
operator|.
