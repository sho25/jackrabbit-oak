...
begin_comment
comment|/**  * FIXME - This is same as CommitCommand except that it does not fetch all valid  * commits upfront. Consolidate the two.  *  * {@code Command} for {@code MongoMicroKernel#commit(String, String, String, String)}  */
end_comment
...
block|}
comment|//    private void readExistingNodes() {
comment|//        FetchNodesActionNew action = new FetchNodesActionNew(nodeStore, affectedPaths,
comment|//                mongoSync.getHeadRevisionId());
comment|//        action.setBranchId(branchId);
comment|//        existingNodes = action.execute();
comment|//    }
comment|// FIXME - Performance, This seems to be faster for commits than the old method.
specifier|private
name|void
name|readExistingNodes
parameter_list|()
...
block|{
comment|// FIXME: this may also mean a node we modify has
comment|// been removed in the meantime
block|}
block|}
block|}
specifier|private
...
name|info
argument_list|(
literal|"Commit @{}: failed due to a concurrent commit."
operator|+
