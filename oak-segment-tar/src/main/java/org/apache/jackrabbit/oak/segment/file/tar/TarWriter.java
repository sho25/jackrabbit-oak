...
name|monitor
decl_stmt|;
comment|/**      * File handle. Initialized lazily in {@link #writeEntry(UUID, byte[],      * byte[], int, int, GCGeneration)} to avoid creating an extra empty file      * when just reading from the repository. Should only be accessed from      * synchronized code.      */
specifier|private
name|RandomAccessFile
name|access
init|=
...
argument_list|)
expr_stmt|;
comment|// TODO frm Include both the full and tail generation. See OAK-6456.
name|buffer
operator|.
name|putInt
argument_list|(
