...
name|info
argument_list|(
literal|"TarMK revision cleanup marking file for deletion: {}"
argument_list|,
name|file
operator|.
name|getName
...
return|;
block|}
comment|/**      * Returns the cancellation policy for the compaction phase. If the disk      * space was considered insufficient at least once during compaction (or if      * the space was never sufficient to begin with), compaction is considered      * canceled.      *      * @return a flag indicating if compaction should be canceled.      */
specifier|private
name|Supplier
argument_list|<
name|Boolean
...
parameter_list|()
block|{
comment|// The canceled flag can only transition from false (its initial
comment|// value), to true. Once compaction is considered canceled,
comment|// there should be no way to go back.
if|if
condition|(
operator|!
name|sufficientDiskSpace
...
name|warn
argument_list|(
literal|"TarMK compaction was canceled, not enough disk space available."
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|info
argument_list|(
literal|"TarMK compaction detected concurrent commits while compacting. "
operator|+
...
name|info
argument_list|(
literal|"TarMK compaction gave up compacting concurrent commits after "
operator|+
literal|"{} cycles."
argument_list|,
name|cycles
...
name|info
argument_list|(
literal|"TarMK compaction force compacting remaining commits"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|warn
argument_list|(
literal|"TarMK compaction failed to force compact remaining commits. "
operator|+
...
name|info
argument_list|(
literal|"TarMK compaction completed after {} cycles in {}ms"
argument_list|,
name|cycles
operator|-
literal|1
