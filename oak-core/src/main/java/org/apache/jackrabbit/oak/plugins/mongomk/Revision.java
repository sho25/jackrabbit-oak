...
return|;
block|}
comment|/**          * Get the timestamp from the revision range, if found. If no range was          * found for this cluster instance, or if the revision is older than the          * earliest range, then 0 is returned. If the revision is newer than the          * newest range for this cluster instance, then Long.MAX_VALUE is          * returned.          *           * @param r the revision          * @return the revision where it was seen, null if not found,           *      the timestamp plus 1 second for new local revisions;          *      Long.MAX_VALUE for new non-local revisions (meaning 'in the future')          */
specifier|private
name|Revision
name|getRevisionSeen
parameter_list|(
