...
begin_comment
comment|/**  * Represents a configuration of an index.  */
end_comment
...
literal|1000
decl_stmt|;
comment|/**      * Default value for property {@link #maxFieldLength}.      */
specifier|public
specifier|static
specifier|final
name|int
...
literal|1
decl_stmt|;
comment|/**      * Default suggesterUpdateFrequencyMinutes      */
specifier|public
specifier|static
specifier|final
name|int
...
literal|10
decl_stmt|;
comment|/**      * Default no. of facets retrieved      */
specifier|static
specifier|final
name|int
name|DEFAULT_FACET_COUNT
init|=
literal|10
decl_stmt|;
...
name|String
argument_list|>
name|customTikaMimeTypeMappings
decl_stmt|;
specifier|private
specifier|final
name|int
...
specifier|final
name|int
name|suggesterUpdateFrequencyMinutes
decl_stmt|;
specifier|private
specifier|final
name|long
...
name|testMode
decl_stmt|;
comment|/**      * See {@link FulltextIndexConstants#PROP_VALUE_REGEX}      */
specifier|private
specifier|final
name|Pattern
name|propertyRegex
...
block|}
comment|//~--------------------------------------------------------< Builder>
comment|// TODO - this method should be removed after tests don't use it anymore
specifier|public
specifier|static
name|Builder
name|newBuilder
...
name|Builder
block|{
comment|/**          * Default unique id used when no existing uid is defined          * and index is not populated          */
specifier|private
specifier|static
specifier|final
name|String
...
return|;
block|}
comment|// TODO: This method should be abstract... to be done later after tests are updated so that they compile
specifier|protected
name|IndexDefinition
name|createInstance
parameter_list|(
...
name|maxExtractLength
operator|=
name|determineMaxExtractLength
argument_list|()
expr_stmt|;
name|this
operator|.
name|suggesterUpdateFrequencyMinutes
operator|=
name|evaluateSuggesterUpdateFrequencyMinutes
argument_list|(
name|defn
argument_list|,
name|DEFAULT_SUGGESTER_UPDATE_FREQUENCY_MINUTES
...
block|{
comment|// create a random number
name|randomSeed
operator|=
name|UUID
operator|.
...
name|suggestEnabled
operator|=
name|evaluateSuggestionEnabled
argument_list|()
expr_stmt|;
name|this
operator|.
name|spellcheckEnabled
operator|=
name|evaluateSpellcheckEnabled
argument_list|()
expr_stmt|;
name|this
operator|.
...
name|error
argument_list|(
literal|"Config error for index definition at {} . Please correct the index definition "
operator|+
...
return|;
block|}
comment|//TODO Should this method be abstract?
specifier|protected
name|String
name|getDefaultFunctionName
parameter_list|()
...
literal|"fulltext"
return|;
comment|//TODO Should this be FulltextIndexConstants.FUNC_NAME
block|}
specifier|public
name|boolean
...
return|;
block|}
comment|/**      * Check if the index definition is fresh, or (some) indexing has occurred.      *      * WARNING: If there is _any_ hidden node, then it is assumed that      * no reindex is needed. Even if the hidden node is completely unrelated      * and doesn't contain index data (for example the node ":status").      * See also OAK-7991.      *      * @param definition nodestate for Index Definition      * @return true if index has some indexed content      */
specifier|public
specifier|static
name|boolean
name|hasPersistedIndex
...
parameter_list|(
name|boolean
name|disableStoredIndexDefinitionDefault
parameter_list|)
block|{
name|IndexDefinition
operator|.
...
parameter_list|()
block|{
comment|//Can be computed lazily as required only for oak-run indexing for now
name|Set
argument_list|<
name|String
argument_list|>
...
name|PropertyDefinition
argument_list|>
name|nullCheckEnabledProperties
decl_stmt|;
specifier|private
specifier|final
name|List
...
name|PropertyDefinition
argument_list|>
name|notNullCheckEnabledProperties
decl_stmt|;
specifier|private
specifier|final
name|List
...
argument_list|()
operator|||
name|anyNodeScopeIndexedProperty
argument_list|()
expr_stmt|;
name|this
operator|.
...
name|indexesAllNodesOfMatchingType
operator|=
name|areAlMatchingNodeByTypeIndexed
argument_list|()
expr_stmt|;
name|this
operator|.
...
return|;
block|}
comment|/**          * @return name of the base node type.          */
specifier|public
name|String
...
return|;
block|}
comment|/**          * Returns true if the rule has any property definition which enables          * evaluation of fulltext related clauses          */
specifier|public
name|boolean
name|isFulltextEnabled
parameter_list|()
...
return|;
block|}
comment|/**          * Returns true if a fulltext field for the node would be created          * for any node matching the current rule.          */
specifier|public
name|boolean
...
expr_stmt|;
block|}
comment|//In case of a pure nodetype index we just index primaryType and mixins
comment|//and ignore any other property definition
if|if
condition|(
name|nodeTypeIndex
condition|)
...
name|warn
argument_list|(
literal|"Index at [{}] has {} enabled and cannot support other property "
operator|+
literal|"definitions"
argument_list|,
name|indexPath
...
block|}
block|}
comment|// a function index has no other options
continue|continue;
block|}
if|if
condition|(
...
expr_stmt|;
block|}
comment|//Include props with name, boosted and nodeScopeIndex
if|if
condition|(
name|pd
operator|.
...
block|}
block|}
name|ensureNodeTypeIndexingIsConsistent
argument_list|(
name|propDefns
argument_list|,
name|syncProps
...
return|;
block|}
comment|/**          * If jcr:primaryType is indexed but jcr:mixinTypes is not indexed          * then ensure that jcr:mixinTypes is also indexed          */
specifier|private
name|void
name|ensureNodeTypeIndexingIsConsistent
parameter_list|(
...
parameter_list|()
block|{
comment|//Check if there is any nodeScope indexed property as
comment|//for such case a node would always be indexed
for|for
control|(
name|PropertyDefinition
name|pd
...
return|;
block|}
comment|//If there is nullCheckEnabled property which is not relative then
comment|//all nodes would be indexed. relativeProperty with nullCheckEnabled might
comment|//not ensure that (OAK-1085)
for|for
control|(
name|PropertyDefinition
name|pd
...
parameter_list|)
block|{
comment|//check global config first
if|if
condition|(
name|getOptionalValue
argument_list|(
...
return|;
block|}
comment|//iterate over property definitions
for|for
control|(
name|PropertyDefinition
name|pd
...
name|IllegalStateException
argument_list|(
literal|"nt:base based rule cannot have a "
operator|+
literal|"PropertyDefinition with nullCheckEnabled"
argument_list|)
throw|;
block|}
block|}
...
argument_list|)
decl_stmt|;
comment|//To ensure compatibility with previous Tree based usage look based on string also
if|if
condition|(
name|primaryType
operator|==
...
argument_list|()
decl_stmt|;
comment|//A nodetype index just required propertyIndex and sync flags to be set
name|builder
operator|.
name|setProperty
argument_list|(
...
specifier|final
name|int
name|statisticalFacetSampleSize
decl_stmt|;
name|SecureFacetConfiguration
parameter_list|(
name|long
...
block|{
comment|// use "default" just as placeholder. default case would do the actual default eval
name|modeString
operator|=
name|System
operator|.
...
decl_stmt|;
name|int
name|statisticalFacetSampleSizePV
init|=
name|getOptionalValue
argument_list|(
name|facetConfigRoot
