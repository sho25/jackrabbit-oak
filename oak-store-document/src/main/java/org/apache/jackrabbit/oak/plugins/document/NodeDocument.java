...
decl_stmt|;
comment|/**      * The path prefix for previous documents.      */
specifier|private
specifier|static
specifier|final
name|Path
...
return|;
block|}
comment|/**      * Purge the  uncommitted revisions of this document with the      * local cluster node id as returned by the {@link RevisionContext}. These      * are the {@link #REVISIONS} and {@link #BRANCH_COMMITS} entries where      * {@link Utils#isCommitted(String)} returns false.      *      *<p>      *<bold>Note</bold> - This method should only be invoked upon startup      *     as then only we can safely assume that these revisions would not be      *     committed      *</p>      *      * @param context the revision context.      * @return count of the revision entries purged      */
name|int
...
return|;
block|}
comment|// get the commit value if it is not yet available
if|if
condition|(
name|commitValue
operator|==
...
condition|)
block|{
comment|// this change is not committed, hence not valid/visible
return|return
literal|false
return|;
block|}
...
return|;
block|}
comment|/**      * Returns<code>true</code> if the given revision      * {@link Utils#isCommitted(String)} in the revisions map (including      * revisions split off to previous documents) and is visible from the      *<code>readRevision</code>. This includes branch commits if the read      * revision is on the same branch and is equal or newer than the revision      * to check.      *      * @param revision  the revision to check.      * @param commitValue the commit value of the revision to check.      * @param readRevision the read revision.      * @return<code>true</code> if the revision is visible, otherwise      *<code>false</code>.      */
specifier|private
name|boolean
name|isVisible
parameter_list|(
