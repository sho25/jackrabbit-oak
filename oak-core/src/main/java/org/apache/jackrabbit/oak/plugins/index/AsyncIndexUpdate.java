...
name|asyncName
argument_list|,
literal|"async name should not be null"
argument_list|)
expr_stmt|;
if|if
condition|(
...
argument_list|)
argument_list|,
literal|"async name [%s] does not confirm to "
operator|+
literal|"naming pattern of ending with 'async'"
argument_list|,
name|asyncName
argument_list|)
expr_stmt|;
...
block|{
comment|/**          * Interval in terms of number of nodes traversed after which          * time would be checked for lease expiry          */
specifier|public
specifier|static
specifier|final
name|int
name|LEASE_CHECK_INTERVAL
init|=
literal|10
decl_stmt|;
specifier|private
...
name|debug
argument_list|(
literal|"checkpoint cleanup skipped because cleanupIntervalMinutes set to: "
operator|+
name|cleanupIntervalMinutes
argument_list|)
expr_stmt|;
...
name|debug
argument_list|(
literal|"checkpoint cleanup skipped because index stats are failing: "
operator|+
name|indexStats
argument_list|)
expr_stmt|;
...
block|{
comment|// clean up every five minutes by default
name|long
name|currentMinutes
init|=
name|TimeUnit
...
name|debug
argument_list|(
literal|"checkpoint cleanup scheduled in "
operator|+
name|scheduledInMinutes
operator|+
literal|" minutes"
...
parameter_list|)
block|{
comment|//As metrics is an optional library guard the access with the check
if|if
condition|(
name|statisticsProvider
operator|.
...
name|equals
argument_list|(
literal|"MetricStatisticsProvider"
argument_list|)
condition|)
block|{
name|MetricRegistry
