...
argument_list|)
decl_stmt|;
comment|// TODO(frm) This is only exposed to tests. Should it be removed?
specifier|public
specifier|static
specifier|final
name|String
...
name|description
operator|=
literal|"Apache Jackrabbit Oak NodeStore implementation based on "
operator|+
literal|"the segment model. For configuration refer to http://jackrabbit.apache.org/oak/docs/osgi_config.html#SegmentNodeStore. "
operator|+
literal|"Note that for system stability purpose it is advisable to not "
operator|+
literal|"change these settings at runtime. Instead the config change should "
operator|+
literal|"be done via file system based config file and this view should ONLY "
operator|+
literal|"be used to determine which options are supported."
argument_list|)
...
name|name
operator|=
literal|"Split persistence"
argument_list|,
name|description
operator|=
literal|"Boolean value indicating that the writes should be done locally when using the custom segment store"
argument_list|)
name|boolean
