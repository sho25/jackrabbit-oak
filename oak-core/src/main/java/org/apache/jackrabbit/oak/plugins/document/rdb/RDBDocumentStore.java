...
condition|)
block|{
comment|// if we do not have a blob, the first part of the string data is the base JSON
name|baseData
operator|=
operator|(
name|Map
...
name|tableName
operator|+
literal|" set MODIFIED = ?, HASBINARY = ?, MODCOUNT = ?, CMODCOUNT = ?, DSIZE = DSIZE + ?, DATA = DATA || ? where ID = ?"
decl_stmt|;
if|if
condition|(
name|oldmodcount
