...
block|}
block|}
comment|/**      * Ensures a primary node type definition that does not extend from any      * other primary node type has {@code nt:base} in the {@code jcr:supertypes}      * list. Listing {@code nt:base} in this case is not mandatory in a CND, but      * is required in {@code jcr:supertypes}.      *      * @param types the parent node for all node type definitions.      * @param type the node type definition to process.      */
specifier|private
name|void
name|ensureNtBase
parameter_list|(
...
return|return;
block|}
comment|// This is a primary node type.
comment|// Make sure jcr:supertypes contains nt:base when needed.
name|Iterable
argument_list|<
name|String
argument_list|>
...
block|{
comment|// is any of the supertypes a primary node type?
name|boolean
name|addNtBase
init|=
literal|true
