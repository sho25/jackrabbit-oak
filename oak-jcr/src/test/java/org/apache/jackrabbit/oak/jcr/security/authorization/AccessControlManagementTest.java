...
annotation|@
name|Test
comment|// TODO: check if this duplicates the next test
specifier|public
name|void
name|testAccessControlRead
parameter_list|()
throws|throws
name|Exception
block|{
...
comment|// ok.
block|}
comment|/* Finally the test user must not be allowed to remove the policy. */
try|try
block|{
name|testAcMgr
operator|.
...
argument_list|)
decl_stmt|;
comment|/*          testuser must not have          - permission to view AC items          - permission to modify AC items         */
comment|// make sure the 'rep:policy' node has been created.
name|assertTrue
argument_list|(
name|superuser
...
argument_list|)
expr_stmt|;
comment|// the policy node however must not be visible to the test-user
name|assertFalse
argument_list|(
name|testSession
operator|.
...
specifier|public
name|void
name|testAccessControlModification2
parameter_list|()
throws|throws
name|Exception
block|{
...
name|fail
argument_list|(
literal|"MODIFY_AC privilege must not apply outside of the tree it has applied to."
argument_list|)
expr_stmt|;
block|}
catch|catch
