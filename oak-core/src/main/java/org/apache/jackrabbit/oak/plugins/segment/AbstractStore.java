...
condition|)
block|{
comment|// no caching, just load the segment directly
try|try
block|{
return|return
name|loadSegment
...
name|IllegalStateException
argument_list|(
literal|"Unable to find segment "
operator|+
name|id
argument_list|)
throw|;
...
argument_list|()
expr_stmt|;
comment|// for another thread to finish loading the segment
block|}
catch|catch
parameter_list|(
name|InterruptedException
...
argument_list|()
expr_stmt|;
comment|// for other threads to finish loading segments
block|}
catch|catch
parameter_list|(
name|InterruptedException
