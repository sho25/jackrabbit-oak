...
literal|true
decl_stmt|;
comment|// FIXME OAK-3348 re-enable caches but make generation part of cache key to avoid backrefs
specifier|final
name|int
name|safeSize
init|=
...
expr_stmt|;
block|}
comment|// FIXME OAK-3348 this is a hack and probably prone to races: replace with making the tacker allocate a new writer
specifier|public
name|void
name|dropCache
parameter_list|()
...
name|IOException
block|{
comment|// Not implementing Closeable because we are not idempotent
name|segmentBufferWriterPool
operator|.
name|returnWriter
argument_list|(
...
return|;
block|}
comment|// FIXME OAK-3348 defer compacted items are not in the compaction map -> performance regression
comment|//        split compaction map into 1) id based equality and 2) cache (like string and template) for nodes
specifier|private
name|SegmentNodeState
name|writeNode
parameter_list|(
