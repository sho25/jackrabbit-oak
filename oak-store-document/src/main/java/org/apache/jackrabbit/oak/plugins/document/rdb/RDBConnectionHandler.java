begin_unit|revision:0.9.5;language:Java;cregit-version:0.0.1
begin_comment
comment|/*  * Licensed to the Apache Software Foundation (ASF) under one or more  * contributor license agreements.  See the NOTICE file distributed with  * this work for additional information regarding copyright ownership.  * The ASF licenses this file to You under the Apache License, Version 2.0  * (the "License"); you may not use this file except in compliance with  * the License.  You may obtain a copy of the License at  *  *      http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing, software  * distributed under the License is distributed on an "AS IS" BASIS,  * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.  * See the License for the specific language governing permissions and  * limitations under the License.  */
end_comment

begin_package
package|package
name|org
operator|.
name|apache
operator|.
name|jackrabbit
operator|.
name|oak
operator|.
name|plugins
operator|.
name|document
operator|.
name|rdb
package|;
end_package

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|Closeable
import|;
end_import

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|IOException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|lang
operator|.
name|ref
operator|.
name|WeakReference
import|;
end_import

begin_import
import|import
name|java
operator|.
name|sql
operator|.
name|Connection
import|;
end_import

begin_import
import|import
name|java
operator|.
name|sql
operator|.
name|SQLException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|concurrent
operator|.
name|ConcurrentHashMap
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|concurrent
operator|.
name|ConcurrentMap
import|;
end_import

begin_import
import|import
name|javax
operator|.
name|sql
operator|.
name|DataSource
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|jackrabbit
operator|.
name|oak
operator|.
name|plugins
operator|.
name|document
operator|.
name|DocumentStoreException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|jetbrains
operator|.
name|annotations
operator|.
name|NotNull
import|;
end_import

begin_import
import|import
name|org
operator|.
name|jetbrains
operator|.
name|annotations
operator|.
name|Nullable
import|;
end_import

begin_import
import|import
name|org
operator|.
name|slf4j
operator|.
name|LoggerFactory
import|;
end_import

begin_comment
comment|/**  * Utility functions for connection handling.  */
end_comment

begin_class
specifier|public
class|class
name|RDBConnectionHandler
implements|implements
name|Closeable
block|{
specifier|private
name|DataSource
name|ds
decl_stmt|;
specifier|private
name|long
name|closedTime
init|=
literal|0L
decl_stmt|;
specifier|private
specifier|static
specifier|final
name|org
operator|.
name|slf4j
operator|.
name|Logger
name|LOG
init|=
name|LoggerFactory
operator|.
name|getLogger
argument_list|(
name|RDBConnectionHandler
operator|.
name|class
argument_list|)
decl_stmt|;
comment|/**      * Closing a connection doesn't necessarily imply a {@link Connection#commit()} or {@link Connection#rollback()}.      * This becomes a problem when the pool implemented by the {@link DataSource} re-uses the connection, and may      * affect subsequent users of that connection. This system property allows to enable a check to be done upon      * {@link #closeConnection(Connection)} so that problems can be detected early rather than late.      * See also https://issues.apache.org/jira/browse/OAK-2337.      */
specifier|private
specifier|static
specifier|final
name|boolean
name|CHECKCONNECTIONONCLOSE
init|=
name|Boolean
operator|.
name|getBoolean
argument_list|(
literal|"org.apache.jackrabbit.oak.plugins.document.rdb.RDBConnectionHandler.CHECKCONNECTIONONCLOSE"
argument_list|)
decl_stmt|;
specifier|public
name|RDBConnectionHandler
parameter_list|(
annotation|@
name|NotNull
name|DataSource
name|ds
parameter_list|)
block|{
name|this
operator|.
name|ds
operator|=
name|ds
expr_stmt|;
block|}
comment|/**      * Obtain a {@link Connection} suitable for read-only operations.      */
specifier|public
annotation|@
name|NotNull
name|Connection
name|getROConnection
parameter_list|()
throws|throws
name|SQLException
block|{
name|Connection
name|c
init|=
name|getConnection
argument_list|()
decl_stmt|;
name|c
operator|.
name|setAutoCommit
argument_list|(
literal|false
argument_list|)
expr_stmt|;
name|setReadOnly
argument_list|(
name|c
argument_list|,
literal|true
argument_list|)
expr_stmt|;
return|return
name|c
return|;
block|}
comment|/**      * Obtain a {@link Connection} suitable for read-write operations.      */
specifier|public
annotation|@
name|NotNull
name|Connection
name|getRWConnection
parameter_list|()
throws|throws
name|SQLException
block|{
name|Connection
name|c
init|=
name|getConnection
argument_list|()
decl_stmt|;
name|c
operator|.
name|setAutoCommit
argument_list|(
literal|false
argument_list|)
expr_stmt|;
name|setReadOnly
argument_list|(
name|c
argument_list|,
literal|false
argument_list|)
expr_stmt|;
return|return
name|c
return|;
block|}
comment|/**      * Roll back the {@link Connection}.      */
specifier|public
name|void
name|rollbackConnection
parameter_list|(
annotation|@
name|Nullable
name|Connection
name|c
parameter_list|)
block|{
if|if
condition|(
name|c
operator|!=
literal|null
condition|)
block|{
try|try
block|{
name|c
operator|.
name|rollback
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|SQLException
name|ex
parameter_list|)
block|{
name|LOG
operator|.
name|error
argument_list|(
literal|"error on rollback (ignored)"
argument_list|,
name|ex
argument_list|)
expr_stmt|;
block|}
block|}
block|}
comment|/**      * Close the {@link Connection}.      */
specifier|public
name|void
name|closeConnection
parameter_list|(
name|Connection
name|c
parameter_list|)
block|{
if|if
condition|(
name|c
operator|!=
literal|null
condition|)
block|{
try|try
block|{
if|if
condition|(
name|CHECKCONNECTIONONCLOSE
condition|)
block|{
try|try
block|{
name|setReadOnly
argument_list|(
name|c
argument_list|,
operator|!
name|c
operator|.
name|isReadOnly
argument_list|()
argument_list|)
expr_stmt|;
name|setReadOnly
argument_list|(
name|c
argument_list|,
operator|!
name|c
operator|.
name|isReadOnly
argument_list|()
argument_list|)
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|SQLException
name|ex2
parameter_list|)
block|{
name|LOG
operator|.
name|error
argument_list|(
literal|"got dirty connection"
argument_list|,
name|ex2
argument_list|)
expr_stmt|;
throw|throw
operator|new
name|DocumentStoreException
argument_list|(
literal|"dirty connection on close"
argument_list|,
name|ex2
argument_list|)
throw|;
block|}
block|}
name|c
operator|.
name|close
argument_list|()
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|SQLException
name|ex
parameter_list|)
block|{
name|LOG
operator|.
name|error
argument_list|(
literal|"exception on connection close (ignored)"
argument_list|,
name|ex
argument_list|)
expr_stmt|;
block|}
block|}
block|}
comment|/**      * Return current schema name or {@code null} when unavailable      */
annotation|@
name|Nullable
specifier|public
name|String
name|getSchema
parameter_list|(
name|Connection
name|c
parameter_list|)
block|{
try|try
block|{
return|return
operator|(
name|String
operator|)
name|c
operator|.
name|getClass
argument_list|()
operator|.
name|getMethod
argument_list|(
literal|"getSchema"
argument_list|)
operator|.
name|invoke
argument_list|(
name|c
argument_list|)
return|;
block|}
catch|catch
parameter_list|(
name|Throwable
name|ex
parameter_list|)
block|{
comment|// ignored, it's really best effort
return|return
literal|null
return|;
block|}
block|}
specifier|public
name|boolean
name|isClosed
parameter_list|()
block|{
return|return
name|this
operator|.
name|ds
operator|==
literal|null
return|;
block|}
annotation|@
name|Override
specifier|public
name|void
name|close
parameter_list|()
throws|throws
name|IOException
block|{
name|this
operator|.
name|ds
operator|=
literal|null
expr_stmt|;
name|this
operator|.
name|closedTime
operator|=
name|System
operator|.
name|currentTimeMillis
argument_list|()
expr_stmt|;
block|}
annotation|@
name|NotNull
specifier|private
name|DataSource
name|getDataSource
parameter_list|()
throws|throws
name|IllegalStateException
block|{
name|DataSource
name|result
init|=
name|this
operator|.
name|ds
decl_stmt|;
if|if
condition|(
name|result
operator|==
literal|null
condition|)
block|{
throw|throw
operator|new
name|IllegalStateException
argument_list|(
literal|"Connection handler is already closed ("
operator|+
operator|(
name|System
operator|.
name|currentTimeMillis
argument_list|()
operator|-
name|this
operator|.
name|closedTime
operator|)
operator|+
literal|"ms ago)"
argument_list|)
throw|;
block|}
return|return
name|result
return|;
block|}
annotation|@
name|NotNull
specifier|private
name|Connection
name|getConnection
parameter_list|()
throws|throws
name|IllegalStateException
throws|,
name|SQLException
block|{
name|long
name|ts
init|=
name|System
operator|.
name|currentTimeMillis
argument_list|()
decl_stmt|;
name|dumpConnectionMap
argument_list|(
name|ts
argument_list|)
expr_stmt|;
name|Connection
name|c
init|=
name|getDataSource
argument_list|()
operator|.
name|getConnection
argument_list|()
decl_stmt|;
name|remember
argument_list|(
name|c
argument_list|)
expr_stmt|;
if|if
condition|(
name|LOG
operator|.
name|isDebugEnabled
argument_list|()
condition|)
block|{
name|long
name|elapsed
init|=
name|System
operator|.
name|currentTimeMillis
argument_list|()
operator|-
name|ts
decl_stmt|;
if|if
condition|(
name|elapsed
operator|>=
literal|100
condition|)
block|{
name|LOG
operator|.
name|debug
argument_list|(
literal|"Obtaining a new connection from "
operator|+
name|this
operator|.
name|ds
operator|+
literal|" took "
operator|+
name|elapsed
operator|+
literal|"ms"
argument_list|,
operator|new
name|Exception
argument_list|(
literal|"call stack"
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
return|return
name|c
return|;
block|}
comment|// workaround for broken connection wrappers
comment|// see https://issues.apache.org/jira/browse/OAK-2918
specifier|private
name|Boolean
name|setReadOnlyThrows
init|=
literal|null
decl_stmt|;
comment|// null if we haven't checked yet
specifier|private
name|Boolean
name|setReadWriteThrows
init|=
literal|null
decl_stmt|;
comment|// null if we haven't checked yet
specifier|private
name|void
name|setReadOnly
parameter_list|(
name|Connection
name|c
parameter_list|,
name|boolean
name|ro
parameter_list|)
throws|throws
name|SQLException
block|{
if|if
condition|(
name|ro
condition|)
block|{
if|if
condition|(
name|this
operator|.
name|setReadOnlyThrows
operator|==
literal|null
condition|)
block|{
comment|// we don't know yet, so we try at least once
try|try
block|{
name|c
operator|.
name|setReadOnly
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|this
operator|.
name|setReadOnlyThrows
operator|=
name|Boolean
operator|.
name|FALSE
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|SQLException
name|ex
parameter_list|)
block|{
name|LOG
operator|.
name|error
argument_list|(
literal|"Connection class "
operator|+
name|c
operator|.
name|getClass
argument_list|()
operator|+
literal|" erroneously throws SQLException on setReadOnly(true); not trying again"
argument_list|)
expr_stmt|;
name|this
operator|.
name|setReadOnlyThrows
operator|=
name|Boolean
operator|.
name|TRUE
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
operator|!
name|this
operator|.
name|setReadOnlyThrows
condition|)
block|{
name|c
operator|.
name|setReadOnly
argument_list|(
literal|true
argument_list|)
expr_stmt|;
block|}
block|}
else|else
block|{
if|if
condition|(
name|this
operator|.
name|setReadWriteThrows
operator|==
literal|null
condition|)
block|{
comment|// we don't know yet, so we try at least once
try|try
block|{
name|c
operator|.
name|setReadOnly
argument_list|(
literal|false
argument_list|)
expr_stmt|;
name|this
operator|.
name|setReadWriteThrows
operator|=
name|Boolean
operator|.
name|FALSE
expr_stmt|;
block|}
catch|catch
parameter_list|(
name|SQLException
name|ex
parameter_list|)
block|{
name|LOG
operator|.
name|error
argument_list|(
literal|"Connection class "
operator|+
name|c
operator|.
name|getClass
argument_list|()
operator|+
literal|" erroneously throws SQLException on setReadOnly(false); not trying again"
argument_list|)
expr_stmt|;
name|this
operator|.
name|setReadWriteThrows
operator|=
name|Boolean
operator|.
name|TRUE
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
operator|!
name|this
operator|.
name|setReadWriteThrows
condition|)
block|{
name|c
operator|.
name|setReadOnly
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
block|}
block|}
specifier|private
specifier|static
class|class
name|ConnectionHolder
block|{
specifier|public
name|String
name|thread
decl_stmt|;
specifier|public
name|String
name|caller
decl_stmt|;
specifier|public
name|long
name|ts
decl_stmt|;
specifier|public
name|ConnectionHolder
parameter_list|()
block|{
name|Thread
name|t
init|=
name|Thread
operator|.
name|currentThread
argument_list|()
decl_stmt|;
name|this
operator|.
name|thread
operator|=
name|t
operator|.
name|getName
argument_list|()
expr_stmt|;
name|this
operator|.
name|caller
operator|=
name|getCaller
argument_list|(
name|t
operator|.
name|getStackTrace
argument_list|()
argument_list|)
expr_stmt|;
name|this
operator|.
name|ts
operator|=
name|System
operator|.
name|currentTimeMillis
argument_list|()
expr_stmt|;
block|}
specifier|public
name|long
name|getTimestamp
parameter_list|()
block|{
return|return
name|ts
return|;
block|}
specifier|public
name|String
name|dump
parameter_list|(
name|long
name|now
parameter_list|)
block|{
return|return
literal|"(thread="
operator|+
name|thread
operator|+
literal|", caller="
operator|+
name|caller
operator|+
literal|", age="
operator|+
operator|(
name|now
operator|-
name|ts
operator|)
operator|+
literal|")"
return|;
block|}
block|}
specifier|private
specifier|final
name|int
name|LOGTHRESHOLD
init|=
literal|20
decl_stmt|;
specifier|private
name|ConcurrentMap
argument_list|<
name|WeakReference
argument_list|<
name|Connection
argument_list|>
argument_list|,
name|ConnectionHolder
argument_list|>
name|connectionMap
init|=
operator|new
name|ConcurrentHashMap
argument_list|<>
argument_list|()
decl_stmt|;
specifier|private
name|void
name|dumpConnectionMap
parameter_list|(
name|long
name|ts
parameter_list|)
block|{
if|if
condition|(
name|LOG
operator|.
name|isTraceEnabled
argument_list|()
condition|)
block|{
name|connectionMap
operator|.
name|forEach
argument_list|(
parameter_list|(
name|k
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
try|try
block|{
name|Connection
name|con
init|=
name|k
operator|.
name|get
argument_list|()
decl_stmt|;
if|if
condition|(
name|con
operator|==
literal|null
operator|||
name|con
operator|.
name|isClosed
argument_list|()
condition|)
block|{
name|connectionMap
operator|.
name|remove
argument_list|(
name|k
argument_list|)
expr_stmt|;
block|}
block|}
catch|catch
parameter_list|(
name|SQLException
name|ex
parameter_list|)
block|{                 }
block|}
argument_list|)
expr_stmt|;
name|int
name|size
init|=
name|connectionMap
operator|.
name|size
argument_list|()
decl_stmt|;
if|if
condition|(
name|size
operator|>
literal|0
condition|)
block|{
name|int
name|cnt
init|=
literal|0
decl_stmt|;
name|StringBuilder
name|sb
init|=
operator|new
name|StringBuilder
argument_list|()
decl_stmt|;
for|for
control|(
name|ConnectionHolder
name|ch
range|:
name|connectionMap
operator|.
name|values
argument_list|()
control|)
block|{
if|if
condition|(
name|ts
operator|-
name|ch
operator|.
name|getTimestamp
argument_list|()
operator|>=
name|LOGTHRESHOLD
condition|)
block|{
if|if
condition|(
name|cnt
operator|!=
literal|0
condition|)
block|{
name|sb
operator|.
name|append
argument_list|(
literal|", "
argument_list|)
expr_stmt|;
block|}
name|cnt
operator|+=
literal|1
expr_stmt|;
name|sb
operator|.
name|append
argument_list|(
name|ch
operator|.
name|dump
argument_list|(
name|ts
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|cnt
operator|>
literal|0
condition|)
block|{
name|LOG
operator|.
name|trace
argument_list|(
name|cnt
operator|+
literal|" connections with age>= "
operator|+
name|LOGTHRESHOLD
operator|+
literal|"ms active while obtaining new connection: "
operator|+
name|sb
operator|.
name|toString
argument_list|()
argument_list|)
expr_stmt|;
block|}
block|}
block|}
block|}
specifier|private
name|void
name|remember
parameter_list|(
name|Connection
name|c
parameter_list|)
block|{
if|if
condition|(
name|LOG
operator|.
name|isTraceEnabled
argument_list|()
condition|)
block|{
name|connectionMap
operator|.
name|put
argument_list|(
operator|new
name|WeakReference
argument_list|<
name|Connection
argument_list|>
argument_list|(
name|c
argument_list|)
argument_list|,
operator|new
name|ConnectionHolder
argument_list|()
argument_list|)
expr_stmt|;
block|}
block|}
specifier|private
specifier|static
name|String
name|getCaller
parameter_list|(
name|StackTraceElement
index|[]
name|elements
parameter_list|)
block|{
name|StringBuilder
name|sb
init|=
operator|new
name|StringBuilder
argument_list|()
decl_stmt|;
name|String
name|prevClass
init|=
literal|null
decl_stmt|;
for|for
control|(
name|StackTraceElement
name|e
range|:
name|elements
control|)
block|{
name|String
name|cn
init|=
name|e
operator|.
name|getClassName
argument_list|()
decl_stmt|;
if|if
condition|(
operator|!
name|cn
operator|.
name|startsWith
argument_list|(
name|RDBConnectionHandler
operator|.
name|class
operator|.
name|getName
argument_list|()
argument_list|)
operator|&&
operator|!
operator|(
name|cn
operator|.
name|startsWith
argument_list|(
name|Thread
operator|.
name|class
operator|.
name|getName
argument_list|()
argument_list|)
operator|)
condition|)
block|{
if|if
condition|(
name|sb
operator|.
name|length
argument_list|()
operator|!=
literal|0
condition|)
block|{
name|sb
operator|.
name|append
argument_list|(
literal|" "
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|e
operator|.
name|getClassName
argument_list|()
operator|.
name|equals
argument_list|(
name|prevClass
argument_list|)
condition|)
block|{
name|String
name|loc
decl_stmt|;
if|if
condition|(
name|e
operator|.
name|isNativeMethod
argument_list|()
condition|)
block|{
name|loc
operator|=
literal|"Native Method"
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|e
operator|.
name|getFileName
argument_list|()
operator|==
literal|null
condition|)
block|{
name|loc
operator|=
literal|"Unknown Source"
expr_stmt|;
block|}
else|else
block|{
name|loc
operator|=
name|e
operator|.
name|getFileName
argument_list|()
operator|+
literal|":"
operator|+
name|e
operator|.
name|getLineNumber
argument_list|()
expr_stmt|;
block|}
name|sb
operator|.
name|append
argument_list|(
literal|'.'
argument_list|)
operator|.
name|append
argument_list|(
name|e
operator|.
name|getMethodName
argument_list|()
argument_list|)
operator|.
name|append
argument_list|(
literal|'('
argument_list|)
operator|.
name|append
argument_list|(
name|loc
argument_list|)
operator|.
name|append
argument_list|(
literal|')'
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|sb
operator|.
name|append
argument_list|(
name|e
operator|.
name|toString
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|prevClass
operator|=
name|e
operator|.
name|getClassName
argument_list|()
expr_stmt|;
block|}
block|}
return|return
name|sb
operator|.
name|toString
argument_list|()
return|;
block|}
block|}
end_class

end_unit

