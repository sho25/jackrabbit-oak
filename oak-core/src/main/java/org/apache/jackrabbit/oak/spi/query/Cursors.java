...
return|;
block|}
comment|/**      * Creates a cursor which wraps another cursor and adds a path prefix to      * each of row of the wrapped cursor. This method will return the passed      * cursor as is if {@code path} is the empty string or the root path ("/").      *      * @param c    the cursor to wrap.      * @param path the path prefix.      * @return the cursor.      */
specifier|public
specifier|static
name|Cursor
...
block|}
block|}
comment|/**      * A cursor which wraps another cursor and adds a path prefix to each of      * row of the wrapped cursor.      */
specifier|private
specifier|static
specifier|final
class|class
