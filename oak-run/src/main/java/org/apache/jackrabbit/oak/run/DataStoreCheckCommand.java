...
operator|+
literal|" [--dump<path>] [--repoHome<repo_home>] [--track] "
operator|+
literal|"[--verbose] [--verboseRootPath<verbose_root_path>]"
decl_stmt|;
try|try
init|(
name|Closer
...
argument_list|)
decl_stmt|;
comment|// Optional argument to specify root path under which tracking if to be done. Defaults to "/" if not specified
name|ArgumentAcceptingOptionSpec
...
condition|)
block|{
comment|// Find blob ids by traversal for verbose mode + Segment Store or if verboseRootPath option
comment|// is present (find blob references under a specific root path.)
if|if
condition|(
operator|(
name|options
...
argument_list|()
expr_stmt|;
comment|// Ignore inline encoded binaries in document mk and null references in segment mk
if|if
condition|(
name|id
operator|==
...
argument_list|()
expr_stmt|;
comment|// Ignore inline encoded binaries in document mk
if|if
condition|(
name|id
operator|==
