...
argument_list|)
expr_stmt|;
comment|//TODO Flag out invalid propertyNames like one which are absolute
name|this
operator|.
name|relativePropsMapping
operator|=
name|collectRelativeProps
argument_list|(
name|Iterables
operator|.
name|concat
...
name|relativeProperties
operator|=
name|collectRelativeProperties
argument_list|(
name|definedIndexRules
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**      * Collects the relative properties where the property name matches given name. Note      * that multiple relative properties can end with same name e.g. foo/bar, baz/bar      *      * @param name property name without path      * @param relProps matching relative properties where the relative property path ends      *                 with given name      */
specifier|public
name|void
name|collectRelPropsForName
parameter_list|(
name|String
name|name
parameter_list|,
...
name|fulltextEnabled
decl_stmt|;
comment|//TODO To be relooked with support for aggregation
specifier|final
name|Map
argument_list|<
name|String
...
argument_list|)
expr_stmt|;
comment|//TODO Provide a new proper propertyName for enabling storage
name|this
operator|.
name|defaultStorageEnabled
operator|=
...
parameter_list|)
block|{
comment|//TODO Rules related to type inclusion need to be synced
comment|//Login IndexEditor evaluates differently compared to IndexDefinition
if|if
condition|(
name|propertyTypes
operator|==
...
expr_stmt|;
block|}
comment|/**          * @param path property name to match          * @return<code>true</code> if<code>property name</code> matches this name          *         pattern;<code>false</code> otherwise.          */
name|boolean
name|matches
parameter_list|(
name|String
...
parameter_list|)
block|{
comment|//TODO Remove existing config once transformed config is persisted
name|NodeState
name|rulesState
init|=
name|definition
...
argument_list|)
expr_stmt|;
comment|//indexDefn.removeProperty(DECLARING_NODE_TYPES);
comment|//indexDefn.removeProperty(INCLUDE_PROPERTY_NAMES);
comment|//indexDefn.removeProperty(EXCLUDE_PROPERTY_NAMES);
comment|//indexDefn.removeProperty(ORDERED_PROP_NAMES);
block|}
block|}
