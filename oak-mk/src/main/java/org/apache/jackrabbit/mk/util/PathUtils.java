...
begin_comment
comment|/**  * Utility methods to parse a JCR path.  *<p/>  * Each method validates the input, except if the system property  * {packageName}.SKIP_VALIDATION is set, in which case only minimal validation  * takes place within this function, so when the parameter is an illegal path,  * the the result of this method is undefined.  */
end_comment
...
name|PathUtils
block|{
comment|/**      * Controls whether paths passed into methods of this class are validated or      * not. By default, paths are validated for each method call, which      * potentially slows down processing. To disable validation, set the system      * property org.apache.jackrabbit.mk.util.PathUtils.SKIP_VALIDATION.      */
specifier|private
specifier|static
specifier|final
name|boolean
