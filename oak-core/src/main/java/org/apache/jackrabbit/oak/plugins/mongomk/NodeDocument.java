...
expr_stmt|;
block|}
comment|/**      * Gets the value map for the given key. This method is similar to {@link      * #get(String)} but only returns a map instance if the value associated      * with<code>key</code> is a map. The returned value map may span multiple      * documents if the values of the given<code>key</code> were split off to      * {@link #PREVIOUS} documents.      *      * @param key a string key.      * @return the map associated with the key or<code>null</code> if there is      *         no such entry.      */
annotation|@
name|CheckForNull
specifier|public
name|Map
...
return|;
block|}
comment|/**      * Returns previous {@link NodeDocument}, which include entries for the      * property in the given revision.      * If the<code>revision</code> is<code>null</code>, then all previous      * documents are returned.      *      * @param revision the revision to match or<code>null</code>.      * @param property the name of a property.      * @return previous documents.      */
name|Iterable
argument_list|<
name|NodeDocument
argument_list|>
