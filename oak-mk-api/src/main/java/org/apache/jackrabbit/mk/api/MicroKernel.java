...
name|MicroKernelException
function_decl|;
comment|/**      * Returns the JSON diff representation of the changes between the specified      * revisions. The changes will be consolidated if the specified range      * covers intermediary revisions. {@code fromRevisionId} and {@code toRevisionId}      * don't need not be in a specific chronological order.      *<p>      * The {@code path} parameter allows to filter the changes included in the      * JSON diff, i.e. only those changes that affected the subtree rooted at      * {@code path} will be included.      *<p>      * The {@code depth} limit applies to the subtree rooted at {@code path}.      * It allows to limit the depth of the diff, i.e. only changes up to the      * specified depth will be included in full detail. changes at paths exceeding      * the specified depth limit will be reported as {@code ^"/some/path" : {}},      * indicating that there are unspecified changes below that path.      *<table border="1">      *<tr>      *<th>{@code depth} value</th><th>scope of detailed diff</th>      *</tr>      *<tr>      *<td>-1</td><td>no limit will be applied</td>      *</tr>      *<tr>      *<td>0</td><td>changes affecting the properties and child node names of the node at {@code path}</td>      *</tr>      *<tr>      *<td>1</td><td>changes affecting the properties and child node names of the node at {@code path} and its direct descendants</td>      *</tr>      *<tr>      *<td>...</td><td>...</td>      *</tr>      *</table>      *      * @param fromRevisionId a revision id, if {@code null} the current head revision is assumed      * @param toRevisionId   another revision id, if {@code null} the current head revision is assumed      * @param path           optional path filter; if {@code null} or {@code ""}      *                       the default ({@code "/"}) will be assumed, i.e. no      *                       filter will be applied      * @param depth          depth limit; if {@code -1} no limit will be applied      * @return JSON diff representation of the changes      * @throws MicroKernelException if any of the specified revisions doesn't exist or if another error occurs      */
name|String
comment|/* JSON diff */
name|diff
parameter_list|(
...
name|MicroKernelException
function_decl|;
comment|/**      * Reads up to {@code length} bytes of data from the specified blob into      * the given array of bytes where the actual number of bytes read is      * {@code min(length, max(0, blobLength - pos))}.      *<p>      * If the returned value is smaller than {@code length}, no more data is available.      * This method never returns negative values.      *      * @param blobId blob identifier      * @param pos    the offset within the blob      * @param buff   the buffer into which the data is read.      * @param off    the start offset in array {@code buff}      *               at which the data is written.      * @param length the maximum number of bytes to read      * @return the total number of bytes read into the buffer.      * @throws MicroKernelException if the specified blob does not exist or if another error occurs      */
name|int
comment|/* count */
name|read
parameter_list|(
