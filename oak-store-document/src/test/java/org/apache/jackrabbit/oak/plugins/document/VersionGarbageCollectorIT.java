...
comment|// the split revs, otherwise revision GC won't remove the split doc
name|clock
operator|.
name|waitUntil
argument_list|(
...
argument_list|)
decl_stmt|;
comment|// one split doc each on: /foo, /bar and root document
name|assertEquals
argument_list|(
literal|3
argument_list|,
...
literal|false
return|;
comment|// don't report any doc to be
comment|// GC'able
block|}
block|}
argument_list|)
...
argument_list|)
decl_stmt|;
comment|// create and delete node
name|NodeBuilder
name|builder
init|=
name|store
...
name|assertEquals
argument_list|(
literal|"Not all deletable docs got reported on first run"
argument_list|,
literal|1
argument_list|,
name|docCounter
...
argument_list|)
expr_stmt|;
comment|// create and delete another node
name|builder
operator|=
name|store
operator|.
...
argument_list|()
expr_stmt|;
comment|// wait another hour and GC in last 1 hour
name|clock
operator|.
name|waitUntil
argument_list|(
...
specifier|public
name|void
name|gcDefaultNoBranchSplitDoc
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
block|}
comment|// trigger split of /foo
name|store
operator|.
name|runBackgroundUpdateOperations
argument_list|()
expr_stmt|;
comment|// now /foo must have previous docs
name|NodeDocument
name|doc
init|=
name|getDoc
...
argument_list|)
expr_stmt|;
comment|// revision gc must not collect previous doc because sweep did not run
name|VersionGCStats
name|stats
init|=
name|gc
...
argument_list|)
expr_stmt|;
comment|// write something to make sure sweep rev is after the split revs
comment|// otherwise GC won't collect the split doc
name|builder
operator|=
name|store
operator|.
...
argument_list|)
expr_stmt|;
comment|// run full background operations with sweep
name|clock
operator|.
name|waitUntil
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// now sweep rev must be updated and revision GC can collect prev doc
name|stats
operator|=
name|gc
operator|.
...
argument_list|)
decl_stmt|;
comment|// prepare commit that will trigger split
name|Commit
name|c
init|=
name|store
...
argument_list|)
expr_stmt|;
comment|// give the background operations some time to progress
comment|// past the check for split operations
name|Thread
operator|.
name|sleep
argument_list|(
...
expr_stmt|;
block|}
comment|// pick up the changes performed by first store
name|bgOperationsDone
operator|.
name|await
argument_list|()
...
argument_list|()
expr_stmt|;
comment|// read the node /foo from the store that will perform the
comment|// revision garbage collection
name|NodeState
name|state
init|=
name|ns2
...
argument_list|)
expr_stmt|;
comment|// must have the corresponding document in the cache now
name|NodeDocument
name|doc
init|=
name|ds2
...
argument_list|)
expr_stmt|;
comment|// must not yet have previous documents
name|assertTrue
argument_list|(
name|doc
operator|.
...
argument_list|)
expr_stmt|;
comment|// write something else. this will ensure a journal entry is
comment|// pushed on the next background update operation
name|builder
operator|=
name|store
operator|.
...
argument_list|)
expr_stmt|;
comment|// trigger the overdue split on 1:/foo
name|store
operator|.
name|runBackgroundOperations
argument_list|()
...
argument_list|()
expr_stmt|;
comment|// wait some time and trigger RGC
name|clock
operator|.
name|waitUntil
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// check how the document looks like, bypassing cache
name|doc
operator|=
name|store
operator|.
