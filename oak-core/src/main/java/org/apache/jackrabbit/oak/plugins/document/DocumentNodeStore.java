...
condition|)
block|{
comment|// first lookup with read revisions of nodes and without loader
name|jsop
operator|=
name|diffCache
operator|.
...
literal|false
expr_stmt|;
comment|// fall back to last revisions with loader, this
comment|// guarantees we get a diff
name|jsop
operator|=
name|diffCache
operator|.
...
condition|)
block|{
comment|//Local diff cache support not enabled
return|return
literal|null
return|;
block|}
...
name|trace
argument_list|(
literal|"Got diff from local cache for path {}"
argument_list|,
name|nodeState
operator|.
name|getPath
...
condition|)
block|{
comment|// TODO implement correct behavior if the node doesn't/didn't exist
name|String
name|msg
init|=
name|String
...
name|debug
argument_list|(
literal|"Diff performed via '{}' at [{}] between revisions [{}] => [{}] took {} ms ({} ms)"
argument_list|,
name|diffAlgo
argument_list|,
name|from
