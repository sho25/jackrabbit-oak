...
name|OAK_SECURITY_NAME
operator|+
literal|"=org.apache.jackrabbit.oak.spi.security.authorization.principalbased.impl.PrincipalBasedAuthorizationConfiguration"
argument_list|)
annotation|@
name|Designate
argument_list|(
...
name|name
operator|=
literal|"Apache Jackrabbit Oak Principal Based AuthorizationConfiguration"
argument_list|)
...
name|name
operator|=
literal|"Enable AggregationFilter"
argument_list|,
name|description
operator|=
literal|"If enabled effective permission evaluation will stop after this module."
argument_list|)
name|boolean
...
function_decl|;
block|}
comment|/**      * Reference to service implementing {@link FilterProvider} to define the principals for which this module should take effect.      */
specifier|private
name|FilterProvider
name|filterProvider
decl_stmt|;
comment|/**      * Reference to service implementing {@link MountInfoProvider}      */
specifier|private
name|MountInfoProvider
name|mountInfoProvider
decl_stmt|;
specifier|private
name|ServiceRegistration
name|aggregationFilterRegistration
decl_stmt|;
annotation|@
...
condition|)
block|{
name|registerAggregationFilter
argument_list|(
name|context
argument_list|)
expr_stmt|;
...
block|{
name|unregisterAggregationFilter
argument_list|()
expr_stmt|;
block|}
block|}
...
block|}
comment|//--------------------------------------------------------------------------
comment|/**      * While it is perfectly valid if the filter root is the start of or located below a mount, it's illegal if a given      * mount would start somewhere in the subtree of the filter root distributing the principal based policies between      * different mounts.      */
specifier|private
name|void
name|checkConflictingMount
parameter_list|()
...
argument_list|(
literal|"Mount found below filter root "
operator|+
name|filterRoot
argument_list|)
throw|;
...
name|stream
argument_list|,
literal|"node types for principal based authorization"
argument_list|)
expr_stmt|;
return|return
literal|true
...
name|IllegalStateException
argument_list|(
literal|"Unable to read node types for principal based authorization"
argument_list|,
name|e
argument_list|)
throw|;
