...
specifier|final
name|int
name|STRINGSORT_OVERFLOW_TO_DISK_THRESHOLD
init|=
literal|1024
operator|*
literal|1024
...
comment|// and that one we must now append to the diffcache entry:
while|while
condition|(
name|node
operator|!=
...
expr_stmt|;
block|}
comment|// and finally: mark the diffcache entry as 'done':
name|entry
operator|.
name|done
argument_list|()
...
throw|;
block|}
comment|//TODO: could this also be a problem with very large number of branches ???
name|commits
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// then path denotes the same as myself, hence return myself
return|return
name|this
return|;
block|}
...
name|IllegalStateException
argument_list|(
literal|"path not child of myself. path: "
operator|+
name|path
operator|+
literal|", myself: "
...
condition|)
block|{
comment|// root is parent of everything
return|return
literal|true
return|;
block|}
...
condition|)
block|{
comment|// then I'm not parent of that path
return|return
literal|false
return|;
block|}
...
condition|)
block|{
comment|// if the 'sub' part contains a / then
comment|// it is not a direct child of myself,
comment|// so I'm a grand-parent but not a direct-parent
return|return
literal|false
return|;
block|}
