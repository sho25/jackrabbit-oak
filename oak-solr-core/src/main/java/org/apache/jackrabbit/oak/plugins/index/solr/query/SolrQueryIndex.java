...
specifier|final
name|OakSolrConfigurationProvider
name|fallbackOakSolrConfigurationProvider
decl_stmt|;
specifier|private
specifier|final
name|SolrServerProvider
name|fallbackSolrServerProvider
decl_stmt|;
specifier|private
specifier|static
specifier|final
...
name|warn
argument_list|(
literal|"Cannot open Solr Index at path {} as the index is not of type 'solr'"
argument_list|,
name|path
argument_list|)
expr_stmt|;
...
name|error
argument_list|(
literal|"Could not access the Solr index at "
operator|+
name|path
argument_list|,
name|e
...
control|)
block|{
comment|// all highlighted values end up in 'rep:excerpt', regardless of field match
for|for
control|(
name|String
name|v
...
expr_stmt|;
block|}
comment|// filter facets on doc paths
if|if
condition|(
operator|!
name|facetFields
...
block|{
comment|// remove the entire entry
name|toRemove
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// we can't order by functions
comment|// so remove those entries from the plan's sort order
name|ArrayList
argument_list|<
name|OrderEntry
argument_list|>
