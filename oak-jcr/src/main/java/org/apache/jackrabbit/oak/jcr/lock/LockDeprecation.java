...
name|MAYBEOK
init|=
literal|"(invocation might be ok in order to detect locking support)"
decl_stmt|;
specifier|private
name|LockDeprecation
parameter_list|()
...
argument_list|)
decl_stmt|;
comment|// classes that we don't want to include in the stack trace elements in the
comment|// log message
specifier|private
specifier|static
specifier|final
...
argument_list|)
decl_stmt|;
comment|// classes for which we tolerate locking related calls for now (only TRACE
comment|// level, no WARNING)
specifier|private
specifier|static
specifier|final
name|Set
