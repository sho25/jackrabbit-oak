...
argument_list|()
expr_stmt|;
comment|// wait twice the time we remember revision order
name|clock
operator|.
name|waitUntil
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// collect everything older than one hour (time revision order is remembered)
...
argument_list|)
expr_stmt|;
comment|// trigger purge of revisions older than one hour in RevisionComparator
comment|// this is usually done by the background read operation, but we
comment|// do it explicitly here to make sure it really happens in this test
name|ns2
operator|.
name|getRevisionComparator
argument_list|()
