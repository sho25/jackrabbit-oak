...
return|;
block|}
comment|/**      * Create a dummy cluster node info instance to be utilized for read only access to underlying store.      * @param store      * @return the cluster node info      */
specifier|public
specifier|static
name|ClusterNodeInfo
...
continue|continue;
block|}
comment|// remove entries with "random:" keys if not in use (no lease at all)
if|if
condition|(
name|mId
operator|.
...
name|debug
argument_list|(
literal|"Cleaned up cluster node info for clusterNodeId {} [machineId: {}, leaseEnd: n/a]"
argument_list|,
name|id
argument_list|,
name|mId
...
condition|)
block|{
comment|// we removed it, so we can't re-use it after all
name|alreadyExistingConfigured
operator|=
literal|null
expr_stmt|;
