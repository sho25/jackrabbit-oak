...
argument_list|)
decl_stmt|;
comment|/**      * Minimal interval in milli seconds between subsequent garbage collection cycles.      * Garbage collection invoked via {@link #gc()} will be skipped unless at least      * the specified time has passed since its last successful invocation.      */
specifier|private
specifier|static
specifier|final
name|long
...
name|warn
argument_list|(
literal|"Unable to release the file lock"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|// The TarWriter might have become a TarReader in the
comment|// meantime. Moreover, the TarWriter that became a TarReader
comment|// might have additional entries. Because of this, we need
comment|// to check the list of TarReaders once more.
name|buffer
operator|=
name|FileStoreUtil
operator|.
...
name|cancelled
decl_stmt|;
comment|/** Timestamp of the last time {@link #gc()} was successfully invoked. 0 if never. */
specifier|private
name|long
...
block|}
block|}
comment|/**      * Instances of this class represent the result from a compaction.      * Either {@link #succeeded(int, SegmentGCOptions, RecordId) succeeded},      * {@link #aborted(int, int) aborted} or {@link #skipped(int, SegmentGCOptions) skipped}.      */
specifier|private
specifier|abstract
specifier|static
class|class
...
parameter_list|()
function_decl|;
comment|/**          * @return  {@code true} for {@link #succeeded(int, SegmentGCOptions, RecordId) succeeded}          *          and {@link #skipped(int, SegmentGCOptions) skipped}, {@code false} otherwise.          */
specifier|abstract
name|boolean
name|isSuccess
parameter_list|()
