...
name|indexPath
argument_list|,
literal|"Index path property [%s] not found"
argument_list|,
name|IndexConstants
operator|.
name|INDEX_PATH
...
parameter_list|)
block|{
comment|//Give precedence to count from builder as that reflects the latest state
comment|//and might be higher than one from nodeState which is the base state
if|if
condition|(
name|defnb
operator|!=
...
literal|null
decl_stmt|;
comment|//Check in builder first as that would have latest value
if|if
condition|(
name|defnb
operator|!=
...
expr_stmt|;
block|}
comment|//uid can be null if an old format index has not received any update
return|return
name|uid
return|;
block|}
