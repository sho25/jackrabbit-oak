...
name|getBoolean
argument_list|(
literal|"oak.query.timerDisabled"
argument_list|)
decl_stmt|;
comment|// The sample rate. Must be a power of 2.
specifier|private
specifier|static
specifier|final
name|Long
name|TIMER_SAMPLE_RATE
init|=
name|Long
operator|.
name|getLong
argument_list|(
literal|"oak.query.timerSampleRate"
argument_list|,
literal|0x100
argument_list|)
decl_stmt|;
...
condition|)
block|{
comment|// always measure slow events (slower than 1 ms)
name|measure
argument_list|(
name|timeNanos
argument_list|)
...
condition|)
block|{
comment|// only measure each xth fast event, but multiply by x, so on
comment|// average measured times are correct
name|measure
argument_list|(
name|timeNanos
operator|*
...
condition|)
block|{
comment|// reuse the timer (in the normal case)
name|t
operator|=
name|timerDuration
operator|=
