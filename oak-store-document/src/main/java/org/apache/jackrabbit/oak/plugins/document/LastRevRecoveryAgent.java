...
expr_stmt|;
block|}
comment|// are there branch commits that were merged right before the
comment|// process crashed and the recovery needs to go further back?
comment|// go through branch commits before startTime and check if their
comment|// merge revision is newer than startTime
name|Revision
name|bc
init|=
name|getEarliestBranchCommitMergedAround
argument_list|(
name|root
argument_list|,
name|startTime
...
name|doc
init|=
name|findNearestAncestorOrSelf
argument_list|(
name|parentPath
argument_list|,
name|missingDocuments
...
condition|)
block|{
comment|// there are missing document and the returned document
comment|// does not have bundled nodes
for|for
control|(
name|Path
name|p
...
block|}
comment|//--------------------------< internal>------------------------------------
comment|/**      * Get the earliest branch commit before {@code timeMillis} that has been      * merged after {@code timeMillis}. This method only considers branch      * commits performed by {@code clusterId}.      *      * @param doc the document to check for branch commits.      * @param timeMillis a time in milliseconds since start of the epoch.      * @param clusterId a clusterId.      * @return earliest branch commit or {@code null} if there is none matching      *          the criteria.      */
annotation|@
name|Nullable
specifier|private
name|Revision
