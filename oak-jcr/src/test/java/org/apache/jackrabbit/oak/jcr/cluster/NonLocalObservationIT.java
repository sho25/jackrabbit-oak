...
parameter_list|()
block|{
comment|/**          * Fixes the cluster use case plus allowing to control the cache sizes.          * In theory other users of DocumentMongoFixture might have similar           * test cases - but keeping it simple for now - thus going via subclass.          */
return|return
operator|new
name|DocumentMongoFixture
argument_list|()
...
argument_list|()
decl_stmt|;
comment|/**              * This is not implemented in the super class at all.              *<ul>              *<li>use a specific suffix to make sure we have our own, new db and clean it up after the test</li>              *<li>properly drop that db created above in dispose</li>              *<li>use only 32MB (vs default of 256MB) memory to ensure we're not going OOM just because of this (which happens with the default)</li>              *<li>disable the persistent cache for the same reason</li>              *</ul>              */
annotation|@
name|Override
specifier|public
name|NodeStore
...
argument_list|)
expr_stmt|;
comment|// keep this one low to avoid OOME
name|builder
operator|.
...
name|error
argument_list|(
literal|"dispose: Can't close Mongo"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
block|{
return|return
literal|"NonLocalObservationIT's DocumentMongoFixture flavour"
return|;
block|}
block|}
return|;
...
argument_list|()
expr_stmt|;
comment|// log.info("expectedNodeSuffix:
