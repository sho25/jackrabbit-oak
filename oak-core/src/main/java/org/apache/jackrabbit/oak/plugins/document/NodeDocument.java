...
condition|)
block|{
comment|// baseRev is newer than all previous documents
name|changes
operator|=
name|Iterables
operator|.
...
argument_list|)
decl_stmt|;
comment|// initialize with local revisions and commitRoot entries
name|stack
operator|.
name|addAll
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// no more changes to compare with
if|if
condition|(
name|nextDoc
operator|==
...
return|return;
block|}
comment|// check if current top revision is still newer than
comment|// most recent revision of next document
if|if
condition|(
operator|!
name|stack
...
block|}
block|}
comment|// if we get here, we need to pull in changes
comment|// from nextDoc
name|Iterables
operator|.
name|addAll
argument_list|(
...
block|}
block|}
comment|/**                      * Fetch the next document if {@code nextDoc} is                      * {@code null} and there are more documents.                      */
specifier|private
name|void
name|fetchNextDoc
parameter_list|()
...
block|{
comment|// empty document, try next
name|nextDoc
operator|=
literal|null
expr_stmt|;
...
return|;
block|}
comment|/**      * Returns all changes for the given property back to {@code min} revision      * (exclusive). The revisions include committed as well as uncommitted      * changes.      *      * @param property the name of the property.      * @param min the lower bound revision (exclusive).      * @return changes back to {@code min} revision.      */
annotation|@
name|Nonnull
name|Iterable
argument_list|<
...
block|{
comment|// further revisions with this clusterId
comment|// are older than min revision
name|clusterIds
operator|.
name|remove
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// no more revisions to check
if|if
condition|(
name|clusterIds
operator|.
