...
begin_comment
comment|/**  * The {@code AuthorizableAction} interface provide an implementation  * specific way to execute additional validation or write tasks upon  *  *<ul>  *<li>{@link #onCreate User creation},</li>  *<li>{@link #onCreate Group creation},</li>  *<li>{@link #onRemove Authorizable removal} and</li>  *<li>{@link #onPasswordChange User password modification}.</li>  *</ul>  *  *<p>Please be aware, that in contrast to {@link org.apache.jackrabbit.oak.spi.commit.Validator}  * the authorizable actions will only be enforced when user related content  * modifications are generated by using the user management API.</p>  *  *</p>  *<strong>Note:</strong> user management operations are defined to perform transient  * modifications, which require an explicit save/commit call by the API consumer to  * be persisted. For consistency, implementations of the {@code AuthorizableAction} are expected  * to adhere to this rule and must not 	pre-emptively call {@code Root.commit()}.</p>  *  * @see org.apache.jackrabbit.oak.spi.security.ConfigurationParameters  * @since OAK 1.0  */
end_comment
