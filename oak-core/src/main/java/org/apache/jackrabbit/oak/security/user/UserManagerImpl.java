...
block|{
comment|// type has already been verified upon retrieving the ID, safe to assume this is a group
return|return
operator|new
name|GroupImpl
argument_list|(
...
comment|// for new or imported users, in order to force a pw change upon the next login
if|if
condition|(
name|Utils
operator|.
...
condition|)
block|{
comment|// initial-pw-change: set last-mod property except for new or imported users (irrespective of pw-expiry configuration)
return|return
operator|!
operator|(
name|isImport
...
condition|)
block|{
comment|// only expiry is enabled: set for all user mgt api calls. for user-import only set upon user creation (new tree)
return|return
operator|!
name|isImport
operator|||
...
return|;
block|}
comment|/**      * Select only {@code GroupAction}s from the available {@code AuthorizableAction}s.      *      * @return A {@code List} of {@code GroupAction}s. List may be empty.      */
annotation|@
name|NotNull
specifier|private
name|Iterable
