...
condition|)
block|{
comment|// only validate restrictions if defined with an ACE controlled by this validator. otherwise verify that
comment|// the parent is indeed access control content as defined by the authorization Context.
comment|// this allows alternative authorization models to re-use the configured restriction provide and store
comment|// them in a 'rep:Restriction' node even if the surrounding policy/access control entries look different
if|if
condition|(
name|isAccessControlEntry
argument_list|(
