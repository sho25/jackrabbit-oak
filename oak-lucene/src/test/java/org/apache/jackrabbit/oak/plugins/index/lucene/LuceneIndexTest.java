...
comment|//The way LuceneIndexLookupUtil works is. It collect child first and then
comment|//parent
name|NodeBuilder
name|index
init|=
...
name|assertEquals
argument_list|(
literal|"There must be only one plan"
argument_list|,
literal|1
argument_list|,
name|list
...
name|assertEquals
argument_list|(
literal|"Didn't get the expected plan"
argument_list|,
literal|"/test/oak:index/lucene"
argument_list|,
name|plan
operator|.
name|getPlanName
...
expr_stmt|;
block|}
comment|/**      * Given a lucene index with a config error , it should not block other      * indexes to index content and should log a meaningful Exception . Once      * fixed and reindexed - it should reindex content as expected.      */
annotation|@
name|Test
specifier|public
name|void
...
argument_list|()
decl_stmt|;
comment|// Add some content that qualifies to be indexed by both of the above indexes (separately)
name|builder
operator|.
name|child
argument_list|(
...
init|=
literal|"Unable to get Index Editor for index at /oak:index/luceneTest . "
operator|+
literal|"Please correct the index definition and reindex after correction. "
operator|+
literal|"Additional Info : java.lang.IllegalStateException: Multiple values provided for property entryCount in index definition . Single value was expected"
decl_stmt|;
name|indexed
operator|=
name|HOOK
...
argument_list|)
decl_stmt|;
comment|// Since the index serving property foo has a config error , no plan should be available
name|assertTrue
argument_list|(
name|plans
operator|.
...
argument_list|)
expr_stmt|;
comment|// Now we check the config error in index1 should not impact the query results and content getting indexed for index 2
name|FilterImpl
name|filter2
init|=
name|createFilter
...
argument_list|)
expr_stmt|;
comment|// Now let's fix the config error in index1 and check it now indexes the correct data
name|before
operator|=
name|builder
operator|.
...
argument_list|)
expr_stmt|;
comment|// Since the config error is now fixed - there should not be any more errors here
name|assertTrue
argument_list|(
name|customLogs
operator|.
