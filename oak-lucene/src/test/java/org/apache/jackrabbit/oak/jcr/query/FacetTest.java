...
expr_stmt|;
block|}
comment|/**      * Tests the scenario where we have a facet query and the traversal cost is less than the index cost that      * serves the facet query. TraversingIndex should not be used for facets.      * @throws Exception in case of errors.      */
specifier|public
name|void
name|testTraversalNotAllowed
parameter_list|()
throws|throws
name|Exception
block|{
comment|// Increase cost of lucene index
name|Node
name|luceneGlobal
init|=
name|superuser
...
argument_list|)
expr_stmt|;
comment|// remove test mode as entry count property is ignored in test mode
name|luceneGlobal
operator|.
name|getProperty
argument_list|(
...
argument_list|()
decl_stmt|;
comment|// use issamenode() so that traversal cost comes low
name|String
name|xpath
init|=
literal|"select [rep:facet(text)] from [nt:base] where issamenode('"
operator|+
name|n1
operator|.
name|getPath
