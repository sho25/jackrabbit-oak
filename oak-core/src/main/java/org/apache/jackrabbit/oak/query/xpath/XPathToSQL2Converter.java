...
block|{
comment|/**      * Optimize queries of the form "from [nt:base] where [jcr:primaryType] = 'x'"       * to "from [x] where [jcr:primaryType] = 'x'".      * Enabled by default.      */
specifier|public
specifier|static
specifier|final
name|boolean
name|NODETYPE_OPTIMIZATION
init|=
name|Boolean
operator|.
name|parseBoolean
...
name|getProperty
argument_list|(
literal|"oak.xpathNodeTypeOptimization"
argument_list|,
literal|"true"
argument_list|)
argument_list|)
decl_stmt|;
comment|/**      * Convert queries of the form "where [jcr:primaryType] = 'x' or [jcr:primaryType] = 'y'"      * to "select ... where [jcr:primaryType] = 'x' union select ... where [jcr:primaryType] = 'y'".       * If disabled, only one query with "where [jcr:primaryType] in ('x', 'y') is used.      * Enabled by default.      */
specifier|public
specifier|static
specifier|final
name|boolean
...
name|getProperty
argument_list|(
literal|"oak.xpathNodeTypeUnion"
argument_list|,
literal|"true"
argument_list|)
argument_list|)
...
condition|)
block|{
comment|// probably a path restriction
comment|// String name = readPathSegment();
name|String
name|identifier
init|=
name|readIdentifier
...
block|{
comment|// this will also deal with relative properties
name|Expression
name|e
init|=
...
block|{
comment|// this will also deal with relative properties
comment|// (functions and so on are also working, but this is probably not needed)
name|read
argument_list|(
literal|"("
argument_list|)
...
condition|)
block|{
comment|// only localname(.) and localname() are currently supported
name|read
argument_list|(
literal|"."
argument_list|)
...
return|;
block|}
comment|/**      * Read open bracket (optional), and optional dot, and close bracket.      *       * @param readOpenBracket whether to read the open bracket (false if this      *            was already read)      * @throws ParseException if close bracket or the dot were not read      */
specifier|private
name|void
name|readOpenDotClose
parameter_list|(
name|boolean
name|readOpenBracket
parameter_list|)
throws|throws
name|ParseException
block|{
...
name|queryOptions
expr_stmt|;
comment|// reset fields that are used in the union,
comment|// but no longer in the individual statements
comment|// (can not use clear, because it is shared)
name|stat
operator|.
name|orderList
operator|=
