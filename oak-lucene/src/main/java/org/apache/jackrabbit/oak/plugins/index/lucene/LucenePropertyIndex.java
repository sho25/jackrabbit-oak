...
name|getBoolean
argument_list|(
literal|"oak.lucene.nonLazyIndex"
argument_list|)
decl_stmt|;
specifier|public
specifier|final
...
init|=
literal|"oak.lucene.oldFacetProvider"
decl_stmt|;
specifier|private
specifier|final
specifier|static
name|boolean
name|OLD_FACET_PROVIDER
init|=
name|Boolean
operator|.
name|getBoolean
...
condition|)
block|{
comment|// here the current searcher gets referenced for later
comment|// but the searcher might get closed in the meantime
name|facetProvider
operator|=
operator|new
...
block|{
comment|// a new searcher is opened and closed when needed
name|facetProvider
operator|=
operator|new
name|DelayedLuceneFacetProvider
argument_list|(
name|LucenePropertyIndex
operator|.
name|this
...
condition|)
block|{
comment|// reranking step for fv sim search
name|PropertyRestriction
name|pr
init|=
literal|null
...
block|}
block|}
comment|/**      * A index node implementation that acquires the underlying index only if      * actually needed. This is to avoid downloading the index for the planning      * phase, if there is no chance that the index is actually used.      */
specifier|static
class|class
name|LazyLuceneIndexNode
implements|implements
...
expr_stmt|;
block|}
comment|// to ensure it is not used after releasing
name|indexNode
operator|=
literal|null
expr_stmt|;
...
name|message
init|=
literal|"No index node, corrupt index? "
operator|+
name|indexPath
decl_stmt|;
name|LOG
...
name|indexNode
decl_stmt|;
comment|// double checked locking implemented in the correct way for Java 5
comment|// and newer (actually I don't think this is ever called
comment|// concurrently right now, but better be save)
if|if
condition|(
name|n
operator|==
