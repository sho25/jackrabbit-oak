...
block|}
block|}
comment|// property added or modified?
for|for
control|(
name|PropertyState
name|p
...
argument_list|()
expr_stmt|;
comment|// make sure we don't expose information about a non-accessible property
if|if
condition|(
operator|!
name|hasProperty
...
return|;
block|}
comment|// get status of this tree without checking for it's existence
name|Status
name|nodeStatus
init|=
name|super
...
block|}
block|}
comment|/**      * Internal method for checking whether this node exists and is visible      * (i.e. not hidden).      *      * @return {@true} if the node is visible, {@code false} if not      */
specifier|private
name|boolean
name|isVisible
parameter_list|()
