...
name|String
argument_list|>
name|DEFAULT_EXCLUDE_FRAGMENTS
init|=
name|NONE
decl_stmt|;
specifier|public
...
name|DEFAULT_MERGE_PATHS
decl_stmt|;
comment|/**      * Whether or not to copy binaries by reference. Defaults to false.      */
specifier|private
name|boolean
...
expr_stmt|;
block|}
comment|/**      * Sets the name fragments that should be excluded when the source repository      * is copied to the target repository.      *      * @param excludes Name fragments to be excluded from the copy.      */
specifier|public
name|void
...
argument_list|()
expr_stmt|;
comment|// on TarMK this does call triggers the actual copy
name|logger
operator|.
name|info
argument_list|(
literal|"Upgrading workspace content completed in {}s ({})"
argument_list|,
name|watch
operator|.
name|elapsed
...
name|info
argument_list|(
literal|"Copying version storage"
argument_list|)
expr_stmt|;
name|watch
operator|.
...
name|info
argument_list|(
literal|"Version storage copied in {}s ({})"
argument_list|,
name|watch
operator|.
name|elapsed
...
name|info
argument_list|(
literal|"Skipping the version storage as the copyOrphanedVersions is set to false"
argument_list|)
expr_stmt|;
block|}
name|watch
...
argument_list|)
argument_list|,
comment|// copy referenced version histories
operator|new
name|VersionableEditor
operator|.
name|Provider
...
argument_list|)
expr_stmt|;
comment|// this editor works on the VersionableEditor output, so it can't be
comment|// a part of the same EditorHook
name|hooks
operator|.
name|add
argument_list|(
...
name|info
argument_list|(
literal|"Processing commit hooks completed in {}s ({})"
argument_list|,
name|watch
operator|.
name|elapsed
...
name|info
argument_list|(
literal|"Overriding early shutdown to false because of the copy versions settings"
argument_list|)
expr_stmt|;
return|return
literal|false
...
name|info
argument_list|(
literal|"Overriding early shutdown to false because the target exists"
argument_list|)
expr_stmt|;
return|return
literal|false
...
block|}
block|}
comment|/**      * Copies the registered namespaces to the target repository, and returns      * the internal namespace index mapping used in bundle serialization.      *      * @param targetRoot root builder of the target store      * @param uriToPrefix namespace URI to prefix mapping      * @throws RepositoryException      */
specifier|private
name|void
name|copyNamespaces
parameter_list|(
...
name|debug
argument_list|(
literal|"Privilege {} already exists"
argument_list|,
name|privilegeName
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"Copying workspace {} [i: {}, e: {}, m: {}]"
argument_list|,
name|workspaceName
argument_list|,
name|includes
...
return|;
block|}
comment|// include child nodes from source individually to avoid deleting other initialized content
specifier|final
name|Set
argument_list|<
name|String
...
name|error
argument_list|(
literal|"Node with a long name has been found. Please fix the content or rerun the migration with {} option."
argument_list|,
name|SKIP_NAME_CHECK
argument_list|)
expr_stmt|;
...
name|RepositoryException
argument_list|(
literal|"Node with a long name has been found."
argument_list|)
throw|;
block|}
block|}
