...
specifier|public
class|class
name|MultithreadedOldLuceneFacetProviderReadFailureTest
extends|extends
name|AbstractQueryTest
block|{
annotation|@
...
block|}
block|}
comment|/*         The facet test delays threads intentionally so that index reader closes before reading facets. But sometimes it may fail         because of load on a system. You can change the system property below between 30-50 and see if this test passes on         your system.         The following test is ignored as it is not a consistent test and result may vary based on system load etc.         This test is a race condition and can be test for failure by using different values for  OLD_FACET_PROVIDER_TEST_FAILURE_SLEEP_INSTRUMENT_NAME         The probable value where this will fail is between 30ms to 60ms.         This test's counter part is DelayedFacetReadTest which test new implementation solving the race condition on index reader opening         and closing.      */
annotation|@
name|Ignore
annotation|@
name|Test
...
name|Exception
block|{
comment|// Explicitly setting following configs to run LuceneFacetProvider (old) and a thread sleep of 50 ms in refresh readers. Refer: OAK-8898
name|System
operator|.
name|setProperty
argument_list|(
...
name|contains
argument_list|(
literal|"org.apache.lucene.store.AlreadyClosedException: this IndexReader is closed"
argument_list|)
argument_list|)
expr_stmt|;
name|e
