...
operator|--
expr_stmt|;
comment|// Effect of removed generation is only seen after subsequent cons. See OAK-3317
name|CompactionMap
name|consed
init|=
name|compactionMap
...
expr_stmt|;
block|}
comment|// one final 'cons' to trigger cleanup of empty maps
name|CompactionMap
name|consed
init|=
name|compactionMap
...
argument_list|()
expr_stmt|;
comment|// Find a key not present in the compaction map
name|RecordId
name|key
init|=
name|newRecordId
...
expr_stmt|;
block|}
comment|// Add it and immediately remove it, after which is should be gone
name|compactionMap1
operator|.
name|put
argument_list|(
...
name|assertNull
argument_list|(
literal|"Compaction map must not contain removed key"
argument_list|,
name|compactionMap1
operator|.
name|get
