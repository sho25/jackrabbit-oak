...
block|}
block|}
comment|/**      * Test that concurrent conflicting update to root ends up with a "There's      * already a child" exception in the second try.      */
annotation|@
name|Test
specifier|public
name|void
...
parameter_list|)
block|{
comment|// cmd2 tries to add /a1 again, so this is expected.
block|}
block|}
...
argument_list|)
expr_stmt|;
comment|// To make sure commit /a started waiting.
name|Future
argument_list|<
name|Long
argument_list|>
...
argument_list|)
expr_stmt|;
comment|// To make sure commit /c/d incremented the head revision.
name|Future
argument_list|<
name|Long
argument_list|>
...
block|}
block|}
comment|/**      * A Callable that simply executes the command.      */
specifier|private
specifier|static
class|class
name|CommitCallable
...
block|}
block|}
comment|/**      * A Callable that executes the command with the default command executor.      */
specifier|private
specifier|static
class|class
name|CommitExecutorCallable
