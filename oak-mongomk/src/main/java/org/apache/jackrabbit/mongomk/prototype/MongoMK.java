...
block|{
comment|// TODO currently all children seem to diff,
comment|// which is not necessarily the case
comment|// (compare write counters)
name|w
operator|.
name|tag
argument_list|(
...
expr_stmt|;
block|}
comment|/**      * Get the latest revision where the node was alive at or before the the      * provided revision.      *       * @param nodeMap the node map      * @param maxRev the maximum revision to return      * @return the earliest revision, or null if the node is deleted at the      *         given revision      */
specifier|private
name|Revision
name|getLiveRevision
parameter_list|(
...
return|;
block|}
comment|// public synchronized SortedSet<K> subSet(K fromElement, K toElement) {
comment|//   return keySet.subSet(fromElement, toElement);
comment|// }
block|}
...
condition|)
block|{
comment|// TODO should not happend,
comment|// probably a problem with add/delete/add
name|MicroKernelException
name|e
init|=
operator|new
name|MicroKernelException
argument_list|(
literal|"New child list already cached for id "
operator|+
name|id
argument_list|)
decl_stmt|;
...
name|error
argument_list|(
literal|"Error updating the cache"
argument_list|,
name|e
argument_list|)
expr_stmt|;
