...
specifier|public
name|void
name|setPrimaryTypeWithMandatoryAutoCreatedChild
parameter_list|()
throws|throws
name|Exception
block|{
...
comment|// whether or not the repository scan happens can not be checked
comment|// directly; it requires inspecting the INFO level log, thus the use of
comment|// LogCustomizer
name|LogCustomizer
name|logCustomizer
decl_stmt|;
name|String
...
name|assertEquals
argument_list|(
literal|"captured INFO log should contain exactly one entry, but is: "
operator|+
name|logCustomizer
operator|.
name|getLogs
...
operator|+
literal|" - test:mandatory (string) mandatory"
decl_stmt|;
name|CndImporter
operator|.
name|registerNodeTypes
...
name|fail
argument_list|(
literal|"Must fail with ConstraintViolationException"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
expr_stmt|;
block|}
comment|// remove the mandatory property
name|cnd
operator|=
literal|"<'test'='http://www.apache.org/jackrabbit/test'>\n"
operator|+
...
argument_list|)
expr_stmt|;
comment|// now we should be able to remove the property
name|n
operator|.
name|getProperty
argument_list|(
...
specifier|public
name|void
name|removeMandatoryPropertyFlag
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
block|}
comment|// remove the mandatory property flag
name|cnd
operator|=
literal|"<'test'='http://www.apache.org/jackrabbit/test'>\n"
operator|+
...
specifier|public
name|void
name|addReferenceableToExistingType
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|()
decl_stmt|;
comment|// 1. Create node type that is not referencable
name|String
name|cnd
init|=
literal|"<'test'='http://www.apache.org/jackrabbit/test'> [test:AlmostReferenceable]> nt:base"
decl_stmt|;
name|CndImporter
operator|.
name|registerNodeTypes
...
literal|"testnode"
argument_list|,
literal|"test:AlmostReferenceable"
argument_list|)
decl_stmt|;
name|session
operator|.
...
argument_list|()
expr_stmt|;
comment|// 3. Attempt node type upgrade, adding optional jcr:uuid property
name|cnd
operator|=
literal|"<'test'='http://www.apache.org/jackrabbit/test'> [test:AlmostReferenceable]> nt:base - jcr:uuid (string)"
expr_stmt|;
name|CndImporter
operator|.
name|registerNodeTypes
...
argument_list|()
expr_stmt|;
comment|// property is not a system property yet
name|assertFalse
argument_list|(
name|newnode
operator|.
...
argument_list|)
expr_stmt|;
comment|// 5. Attempt node type upgrade, making the type referencable
name|cnd
operator|=
literal|"<'test'='http://www.apache.org/jackrabbit/test'> [test:AlmostReferenceable]> mix:referenceable, nt:base"
expr_stmt|;
name|CndImporter
operator|.
name|registerNodeTypes
...
argument_list|)
expr_stmt|;
comment|// 6. new node should be referenceable and have jcr:uuid
name|newnode
operator|=
name|root
operator|.
...
argument_list|)
expr_stmt|;
comment|// property is now a system property
name|assertTrue
argument_list|(
name|newnode
operator|.
...
argument_list|)
expr_stmt|;
comment|// 7. try to reference the node
name|Node
name|refnode
init|=
name|root
...
argument_list|()
expr_stmt|;
comment|// 8. get the node by UUID
name|Node
name|found
init|=
name|session
