...
expr_stmt|;
block|}
comment|/**      * Combine the globally defined commit hook(s) and the hooks and validators defined by the      * various security related configurations. In addition a commit hook is added to check      * that the {@code path} to commit contains all unpersisted changes and to fail the      * commit otherwise.      *      * @param path path to commit      * @return A commit hook combining repository global commit hook(s) with the pluggable hooks      *         defined with the security modules and the padded {@code hooks}.      */
specifier|private
name|CommitHook
name|getCommitHook
parameter_list|(
