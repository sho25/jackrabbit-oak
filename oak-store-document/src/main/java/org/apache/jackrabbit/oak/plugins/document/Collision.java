...
expr_stmt|;
block|}
comment|/**      * Marks the collision in the document store. Either our or their      * revision is annotated with a collision marker. Their revision is      * marked if it is not yet committed, otherwise our revision is marked.      *      * @param store the document store.      * @return the revision that was marked. Either our or their.      * @throws DocumentStoreException if the mark operation fails.      * @throws IllegalStateException if neither their nor our revision can be      *              marked because both are already committed.      */
annotation|@
name|NotNull
name|Revision
name|mark
...
condition|)
block|{
comment|// always conflicts because existence changed
return|return
literal|true
return|;
block|}
...
condition|)
block|{
comment|// concurrent change on the property
return|return
literal|true
return|;
block|}
