...
condition|?
literal|null
comment|// FIXME OAK-3349 if no previous compacted base is found we fall back to full compaction by returning null. Add an respective log statement
else|:
name|segmentReader
operator|.
name|readNode
...
parameter_list|()
block|{
comment|// FIXME OAK-3349 the generation needs to reflect the type of compaction: tail or full. Additionally we need to handler the graceful degradation case should #getBase() return null where we would fall back from a tail compaction to a full compaction.
specifier|final
name|GCGeneration
name|newGeneration
init|=
