...
argument_list|)
decl_stmt|;
comment|/**      * The segment write buffer, filled from the end to the beginning      * (see OAK-629). Note that we currently allocate the entire buffer      * right from the beginning. It might turn out that a better approach      * would be to start with a smaller buffer that grows automatically,      * or to use a pool of pre-allocated buffers.      */
specifier|private
specifier|final
name|byte
index|[]
...
argument_list|)
decl_stmt|;
comment|// System.out.println("W: " + entries.size() + " " + level);
name|Preconditions
operator|.
name|checkState
argument_list|(
