...
argument_list|)
decl_stmt|;
comment|//If more than one match found then filter out stale paths
if|if
condition|(
name|allPaths
operator|.
...
name|format
argument_list|(
literal|"Uniqueness constraint violated for property [%s] with value [%s] for "
operator|+
literal|"index [%s]. Indexed paths %s"
argument_list|,
name|propertyRelativePath
argument_list|,
name|value
...
return|;
block|}
comment|/**      * Paths reported by indexes may be based on stale data. So revalidate by checking reported paths are      * valid and refers to indexed value or not      */
specifier|private
name|Set
argument_list|<
name|String
...
continue|continue;
block|}
comment|//Property can be MVP. So check if any of them matches
for|for
control|(
name|String
name|v
