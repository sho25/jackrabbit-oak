...
expr_stmt|;
block|}
comment|/**      * If {@link #getCommittedCredentials()} does not return {@code null} and either {@link #getPrincipals(String)} or      * {@link #getPrincipals(Principal)} has been called during the login/commit phases, this method will removed these      * credentials and principals from the subject or destroy the credentials in case of read-only subject.      *      * If either credentials or principals is {@code null}, this method will return {@code false}, indicating that      * this moduld should be ignored.      *      * @return {@code true} if the logout was successful and {@code false} if this module should be ignored.      * @throws LoginException If the subject is read-only and the committed credentials cannot be destroyed.      * @see #getCommittedCredentials()      * @see<a href="https://docs.oracle.com/javase/8/docs/technotes/guides/security/jaas/JAASLMDevGuide.html#logout>Jaas Dev Guide</a> for additional details.      */
annotation|@
name|Override
specifier|public
name|boolean
...
block|{
comment|// this login module didn't add credentials/principals to the subject upon commit
...
return|;
block|}
comment|/**      * Return the credentials that have been pushed to the {@code Subject} during the {@link #commit()} or {@code null},      * if this login module should be ignored for the {@link #logout()}. Since {@code AbstractLoginModule}      * does not implement the commit-phase, the default implementation returns {@code null}.      *      * @return null. Subclasses of {@link AbstractLoginModule} that modify the subject upon commit should override this      * default implementation returning those credentials that have been extracted during {@link #login() login} and      * added to the subject upon {@link #commit()}.      */
annotation|@
name|Nullable
specifier|protected
name|Credentials
