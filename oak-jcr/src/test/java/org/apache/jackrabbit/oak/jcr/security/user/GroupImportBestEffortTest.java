...
argument_list|)
expr_stmt|;
comment|/*         now try to import the 'g' group that has a circular group         membership references.         expected:         - group is imported         - circular membership is ignored (OR fails upon save)         - g is member of g1         - g1 isn't member of g  (circular membership detected) OR saving changes fails         */
name|doImport
argument_list|(
name|getTargetPath
argument_list|()
...
condition|)
block|{
comment|// circular membership created during import
try|try
block|{
name|getImportSession
argument_list|()
...
name|fail
argument_list|(
literal|"Circular membership must be detected upon save."
argument_list|)
expr_stmt|;
block|}
catch|catch
