...
argument_list|()
expr_stmt|;
comment|// Revision cleanup is still unable to reclaim extra space (OAK-1932)
name|store
operator|=
operator|new
name|FileStore
...
argument_list|)
expr_stmt|;
comment|// FIXME: should be<
name|store
operator|.
name|close
argument_list|()
expr_stmt|;
comment|// Finally do the compaction without concurrent writes
name|store
operator|=
operator|new
name|FileStore
...
argument_list|()
expr_stmt|;
comment|// Now the revision cleanup is able to reclaim the extra space
name|store
operator|=
operator|new
name|FileStore
