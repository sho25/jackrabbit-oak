...
return|;
block|}
comment|/**      * Flushes the current segment to the underlying store, and starts      * writing a new segment. This method is carefully synchronized to avoid      * a deadlock with the flush thread of the underlying segment store.      * The internal variables ({@link #buffer}, etc.) of this class are      * read and updated within a synchronization block, but the actual      * writing of the segment is done outside of that block to allow      * concurrent writing of transient changes and to avoid a deadlock with      * the flush thread that calls this method when already holding the      * lock on the underlying segment store.      */
specifier|public
name|void
name|flush
parameter_list|()
...
block|{
return|return;
comment|// do nothing if there is no content to flush
block|}
comment|// prepare the segment to be written
name|n
operator|=
name|align
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// prepare a new segment for use by any new transient changes
name|uuid
operator|=
name|newDataSegmentId
argument_list|()
...
expr_stmt|;
block|}
comment|// must be outside the synchronization block to avoid a deadlock
name|store
operator|.
name|writeSegment
argument_list|(
...
return|;
block|}
comment|/**      * Prepares the space for the record described in the given arguments.      * This method should be called as the first thing within a synchronization      * block that then proceeds to write the prepared record before leaving      * the synchronization.      *      * @param type type of the record      * @param size number of bytes in the record, excluding the record ids      * @param ids ids of the other records referenced by the record      * @return id of the record being created      */
specifier|private
specifier|synchronized
name|RecordId
name|prepare
