...
name|getLogger
argument_list|(
literal|"org.apache.jackrabbit.oak.audit"
argument_list|)
decl_stmt|;
specifier|static
specifier|final
name|Logger
name|readOperationLogger
init|=
name|LoggerFactory
operator|.
name|getLogger
argument_list|(
literal|"org.apache.jackrabbit.oak.jcr.operations.reads"
argument_list|)
decl_stmt|;
specifier|static
specifier|final
name|Logger
name|writeOperationLogger
init|=
name|LoggerFactory
operator|.
name|getLogger
...
block|}
block|}
comment|/**      * Same as {@link #perform(org.apache.jackrabbit.oak.jcr.session.operation.SessionOperation)}      * but with the option to return {@code null}; thus calling      * {@link org.apache.jackrabbit.oak.jcr.session.operation.SessionOperation#performNullable()}      *      * @param sessionOperation  the {@code SessionOperation} to perform      * @param<T>  return type of {@code sessionOperation}      * @return  the result of {@code sessionOperation.performNullable()}, which      * might also be {@code null}.      * @throws RepositoryException      * @see #perform(org.apache.jackrabbit.oak.jcr.session.operation.SessionOperation)      */
annotation|@
name|Nullable
specifier|public
parameter_list|<
...
block|}
block|}
comment|/**      * Same as {@link #perform(org.apache.jackrabbit.oak.jcr.session.operation.SessionOperation)}      * for calls that don't expect any return value; thus calling      * {@link org.apache.jackrabbit.oak.jcr.session.operation.SessionOperation#performVoid()}.      *      * @param sessionOperation  the {@code SessionOperation} to perform.      * @throws RepositoryException      * @see #perform(org.apache.jackrabbit.oak.jcr.session.operation.SessionOperation)      */
specifier|public
name|void
name|performVoid
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|//For a logout operation the auth info is not accessible
if|if
condition|(
operator|!
name|ops
...
block|{
return|return
literal|"Refresh on observation event"
return|;
block|}
block|}
specifier|private
...
block|{
return|return
literal|"Refresh after a save on the same thread from a different session"
return|;
block|}
block|}
comment|/**      * Read-only RefreshStrategy responsible for notifying the SessionNamespaces      * instance that a refresh was called      */
specifier|private
specifier|static
class|class
name|RefreshNamespaces
